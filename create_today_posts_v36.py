...
# 📌 [수정 완료]: 채널 뿐만 아니라 제목(title)을 기반으로 민간 보험 여부를 완벽히 자동 교차 분류!
def get_category_key(ch_key, title):
    if "실손" in title or "보험" in title:
        return "HEALTH"  # 민간 보험용 템플릿(경고 및 팁 변경) 강제 적용!
...
def make_html_template_clean(p, ch_key):
    ...
    # 📌 제목 기반으로 민간 보험 및 건강/라이프 도메인 교차 판정식 반영
  ...
  - {bullet3}<br>
    ...
    for i in range(5):
        ...
        # 📌 민간 보험/건강 및 라이프/엔터 도메인은 정부 지원금 문구 절대 금지 및 맞춤형 박스 이식
        if cat_key in ["HEALTH", "TECH", "ENTERTAINMENT"]:
            if i == 0:
...
def main():
    print(f"=== [원고 라이터 v36] 도메인 교차 판별 및 실손보험 팩트 무결성 가동 ===")
...
"""
    ...
    if error_log:
        ...
        print(f"🎉 [대성공] 9대 채널 전체 27개 원고가 실손보험 주관 및 약국 조건부 모순까지 모두 클리어하여 최종 무결성 배포 완료!")
