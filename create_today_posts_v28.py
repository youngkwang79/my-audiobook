...
def get_dynamic_docs_and_table(title, keyword):
    if "출산휴가" in title:
        ...
        table = [
            ...
            ["감면 및 지원 한도", "기본적인 기본 공제 혜택 적용", "최대 우대 요율 적용 및 추가 지원금 보전"],
...
def make_html_template_clean(p, ch_key):
    ...
    # 📌 3장 본문 지원 금액 모순 패치 (월 30만원 -> 월 20만원으로 정정)
    if "출산휴가" in title:
  ...
  - {bullet3}<br>
    ...
    for i in range(5):
        ...
        # 📌 [임계 개선]: 유사 문서로 분류되는 원인인 각 메인 챕터 하부의 H3, H4, H5, H6 태그와 중복 문장들을 싹 다 지우고, 
        # 오직 핵심 내용(H2와 그 직후 핵심 설명)만 남겨서 분량을 콤팩트하게 압축합니다!
  ...
  부정한 서식 기재나 자격 초과 이력이 관계 기관의 합동 전산 대조 결과에 따라 사후 적발될 시에는 이미 지급된 지원금 전액의 환수 명령과 함께 법률상 가산금 요율이 가중 소급 부과되므로, 반드시 팩트만을 성실하게 기재하셔야 합니다.
        ...
        # 📌 뜬금없는 금융 대출 볼드 치환 필터 제거 -> 본문 작성 시 키워드만 자연스럽게 강조
...
def main():
    print(f"=== [원고 라이터 v28-CompactNoH3H6-FactFixed] 중복 H3~H6 싹 다 삭제 및 수당 요율 모순 해결 ===")
    ...
    for ch_key in TARGET_CHANNELS:
        ...
        # 1. 3개 포스트 글 본문만 지우고 콤팩트하게 재생성 (thumbnail_prompts.html은 수정/삭제 하지 않고 완벽 보전)
...
"""
    ...
    print(f"\n🎉 [대성공] 중복 문단(H3~H6)이 완전히 지워지고 모순이 완벽 퇴치된 콤팩트 9개 원고가 재생성되었습니다!")
