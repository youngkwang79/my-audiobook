...
def make_html_template(p, ch_key):
    ...
    # 📌 키워드 밀도 보정 처리 (각 텍스트 본문 내에 포커스 키워드를 자연스러운 수식어로 치환하여 삽입)
    # 총 7~10회 분산 매핑이 실현됩니다.
  ...
  - {bullet3}<br>
    ...
    for i in range(5):
        ...
        # 키워드 밀도 보강용 H2별 전용 치환
        p1 = f"{core_fact}\n\n{processed_base_texts[0]}"
        
        # 📌 H3 서브제목에도 키워드 포함되도록 매핑
        sub_h3 = f"### {i+1}.1 {keyword} - {clean_h} 세부 실무 및 자격 쟁점 분석"
...
def main():
    print(f"=== [원고 라이터 v16-SEO] 9대 채널 27개 키워드밀도 보정 원고 컴파일 개시 ===")
...
"""
            ...
            print(f"  ✅ Compiled H1~H6 + SEO Density: post{no:02d}.md ({char_count} chars) & post{no:02d}.html")
            
    print(f"\n🎉 [대성공] 키워드 밀도 최적화 및 원클릭 복사 스크립트가 탑재된 27개 HTML/MD 최종 컴파일 완료!")
