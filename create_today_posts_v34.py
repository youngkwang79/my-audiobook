    ...
    if len(chapters) > 2:
        ...
        for idx1, c1 in enumerate(chapter_contents):
            for idx2, c2 in enumerate(chapter_contents):
                if idx1 != idx2:
                    # 너무 긴 덩어리가 완전히 동일하게 반복되는지 대조
                    c1_clean = re.sub(r'\s+', '', c1)
                    c2_clean = re.sub(r'\s+', '', c2)
                    # 특정 50자 이상의 구절이 100% 동일하게 반복되는지 확인
...
def make_html_template_clean(p, ch_key):
    ...
    # 📌 소상공인 전기요금 신청 시 직접/간접 조건부 구분 패치
    if "전기요금" in title:
        facts[3] = (
...
def main():
    print(f"=== [원고 라이터 v34] Critique Linter 자가 검증 시스템 기동 ===")
...
"""
    ...
    if error_log:
        ...
        print(f"🎉 [대성공] 9대 채널 전체 27개 원고가 Critique Linter 팩트 검수 테스트를 무결성(100% Pass)으로 통과 및 배포되었습니다!")
