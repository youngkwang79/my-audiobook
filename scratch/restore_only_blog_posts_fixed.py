# -*- coding: utf-8 -*-
"""
소설 15종을 works 테이블에서 개별 타겟 영구 삭제(DELETE) 처리하여 배제하고,
오직 21대 진짜 무림북 블로그 지식글과 세무/금융 계산기 3종 (총 24개)만
정상 복원(UPSERT)하는 우회 보강형 스크립트.
"""
...
def run_restore_only_blog_posts_fixed():
    url = ""
    service_key = ""
    ...
    # 1. 15대 소설 목록 타겟 영구 삭제 진행 (소설 폐기!)
    novel_ids = [
        "Woonghon_Kkaedaleumui_Geomgwa_Sonyeon", "Myeolsaguirim Chomuyeong", 
        "namgunguijanhonhaouibigeom", "socheongun3nyenanepamyeleulbee", 
        "hwansaeng-geomjon", "ganghouipaswaeyenoghyelpungdog", 
        "gupailbangcheonjaedeulmumyengs", "gunrimcheonha",
        "cheongun", "yahyelhwamyeng", "sogaju", "cheonhajeil", 
        "hagsasinsin", "mudanggeom", "hwansaenggeomgwi"
    ]

    print("\n[STEP 1] Selectively deleting all 15 novels from DB...")
    for nid in novel_ids:
        del_url = f"{url}/rest/v1/works?id=eq.{nid}"
        req_del = urllib.request.Request(del_url, headers={
            "apikey": service_key,
            "Authorization": f"Bearer {service_key}"
        }, method="DELETE")
        try:
            with urllib.request.urlopen(req_del) as _:
                print(f" ==> [NOVEL PURGED SUCCESS] ID: {nid}")
        except Exception as e:
            print(f" ==> [WARNING] Novel ID {nid} skip: {e}")

    # 2. 오직 진짜 블로그 글 21종 복원 진행
    blog_text_cover_map = {
    ...
    print("\n[STEP 2] Restoring 21 genuine blog posts...")
    for file_name in os.listdir(scratch_dir):
        if file_name.startswith("register_") and file_name.endswith(".js"):
                ...
                if wid in blog_text_cover_map:
                    ...
                    try:
                        with urllib.request.urlopen(req_ups) as _:
                            print(f" ==> [BLOG JS RESTORED] ID: {wid} | '{title}'")
                            restored_count += 1
                    except Exception as e:
            ...
            if title_match and markdown_match and wid in blog_text_cover_map:
                ...
                try:
                    with urllib.request.urlopen(req_ups) as _:
                        print(f" ==> [BLOG PY RESTORED] ID: {wid} | Title: '{title}'")
                        restored_count += 1
                except Exception as e:
    ...
    print(f"\n--- [BLOG-ONLY FIXED RESTORATION COMPLETE] ---")
    print(f"Successfully cleaned all 15 novels and restored exactly {restored_count} blog entries.")

if __name__ == "__main__":
    run_restore_only_blog_posts_fixed()
