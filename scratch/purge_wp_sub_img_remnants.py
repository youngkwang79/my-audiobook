# -*- coding: utf-8 -*-
"""
본문(description) 내에 '{sub_img_html}' 또는 '{sub_img' 기호가 포함되어 유입된
...
def run_purge_wp_sub_img_remnants():
    url = ""
    service_key = ""
    ...
    print("\n[STEP 1] Detecting and deleting posts with '{sub_img_html}' remnants...")
    ...
    try:
        with urllib.request.urlopen(req_query) as res:
            ...
            for w in works_list:
                ...
                # '{sub_img_html}' 또는 '{sub_img_' 키 기호 검색
                has_sub_img_html_remnant = "{sub_img_html}" in desc or "{sub_img_" in desc or "{sub_img" in desc
                
                if has_sub_img_html_remnant:
                    print(f" ==> [DELETE TARGET DETECTED]: '{title}' (ID: {wid})")
                    print("     Reason: Found WordPress automatic sub image placeholder '{sub_img_html}'.")
        ...
        print(f"\n--- [CLEANSING COMPLETE] ---")
        print(f"Successfully purged {deleted_count} remnants containing '{sub_img_html}' from works table.")
        
    except Exception as e:
        print("[ERROR] Cleansing process failed:", e)

if __name__ == "__main__":
    run_purge_wp_sub_img_remnants()
