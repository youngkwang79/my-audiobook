# -*- coding: utf-8 -*-
"""
scratch/ 폴더 내 백업 보관된 모든 python/js 스크립트에서
                ...
                if wid in r2_exact_map:
                    ...
                    try:
                        with urllib.request.urlopen(req_ups) as res_ups:
                            print(f" ==> [FORCED JS RESTORED] ID: {wid} | '{title}'")
                            restored_count += 1
                    except Exception as e:
                        print(f" ==> [ERROR] Failed to restore JS {wid}: {e}")

        # 2. PY 파일 복원 (<img> 및 변수명 전면 정비)
            ...
            if title_match and markdown_match and wid in r2_exact_map:
                ...
                # 💡 [정합성 정비]: 본문 중간의 R2 sub_image 임시 img 태그들은 무림북 순수 마크다운 원칙에 따라 제거
