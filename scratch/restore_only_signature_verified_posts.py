# -*- coding: utf-8 -*-
"""
원래의 진짜 무림북.컴 ID 목록에 해당하는 스크립트들만 읽어와,
본문 내에 '작성자 : sun_writer'가 유실되어 있거나 템플릿으로 쓰인 경우 이를 명확하게 삽입/보정하고
R2 2:3 명품 썸네일과 함께 Supabase works 테이블에 강제 복원(UPSERT)하는 최종 마스터 스크립트.
"""
...
def run_final_unconditional_restoration():
    url = ""
    service_key = ""
    ...
    # 💡 [진짜 무림북 도움되는글 ID 와 R2 2:3 세로형 명품 표지 매핑 테이블]
    r2_exact_map = {
    ...
    for file_name in os.listdir(scratch_dir):
        ...
        if file_name.startswith("register_") and file_name.endswith(".js"):
            ...
            if id_match and markdown_match and title_match:
                ...
                if wid in r2_exact_map:
                    ...
                    # 💡 [정합성 검증]: '작성자 : sun_writer'가 본문 내에 없으면, 최상단 도입부 또는 제목 아래 영역에 완벽 강착 주입!
                    if "sun_writer" not in markdown and "sun-writer" not in markdown:
                        # 도입부 제목 아래 삽입 처리
                        markdown = f"작성자 : sun_writer\n\n" + markdown
                    ...
                    try:
                        with urllib.request.urlopen(req_ups) as res_ups:
                            print(f" ==> [MASTER JS UPSERT] ID: {wid} | '{title}' -> Restored with '작성자 : sun_writer'")
                            restored_count += 1
        ...
        # 2. PY 파일 복원
            ...
            if title_match and markdown_match and wid in r2_exact_map:
                ...
                # 💡 [정합성 검증]: '작성자 : sun_writer'가 본문 내에 없으면, 최상단 도입부 또는 제목 아래 영역에 완벽 강착 주입!
                if "sun_writer" not in markdown_cleaned and "sun-writer" not in markdown_cleaned:
                    markdown_cleaned = f"작성자 : sun_writer\n\n" + markdown_cleaned
                ...
                try:
                    with urllib.request.urlopen(req_ups) as res_ups:
                        print(f" ==> [MASTER PY UPSERT] ID: {wid} | '{title}' -> Restored with '작성자 : sun_writer'")
                        restored_count += 1
                except Exception as e:
                    print(f" ==> [ERROR] Failed to restore PY {wid}: {e}")

    print(f"\n--- [MASTER UNCONDITIONAL RESTORATION COMPLETE] ---")
    print(f"Successfully restored {restored_count} original posts back to their initial state.")

if __name__ == "__main__":
    run_final_unconditional_restoration()
