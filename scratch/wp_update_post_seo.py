# -*- coding: utf-8 -*-
"""
ID 657 포스트의 SEO 메타데이터(스니펫)와 본문 내부 링크를 보완 및 20자 이내 제목으로 교정하는 업데이트 스크립트
"""
...
def get_updated_content():
    # 16:9 본문 이미지 주소를 가져와야 합니다. (이전 656 ID의 URL 사용)
    sub_img_url = "https://blog.murimbook.com/wp-content/uploads/2026/07/time_management_sub_image_1783011053445.jpg"
    ...
    <p>추가적으로, 인생의 성장을 일구는 위대한 리더들의 습관이나 관련 전략에 대해 알아보고 싶다면 <a href="https://blog.murimbook.com/%ec%82%bc%ec%84%b1-%ec%9d%b4%eb%b3%91%ec%b2%a0-%ed%9a%8c%ec%9e%a5%ec%9d%b4-%ea%bf%b0%eb%9a%ab%ec%96%b4-%eb%b3%b8-%eb%b6%80%ec%9e%90-%ec%8a%b5%ea%b4%80/" target="_blank" rel="noopener" style="color: #ffd43b; text-decoration: underline; font-weight: bold;">삼성 이병철 회장 부자 습관 칼럼</a> 글도 매우 훌륭한 인사이트를 제시해 주므로 꼭 일독해 보시기를 권장합니다.</p>
...
def update_post_seo():
    ...
    # 20자 이내의 제목으로 설정 (핵심 키워드를 맨 앞에 위치)
    title = "시간관리 공부법: 집중력 향상 비결" # 딱 19자
    
    content = get_updated_content()
    
    headers = {
        "Content-Type": "application/json"
    }
    
    meta_desc = "시간관리 공부법 핵심 노하우를 공개합니다. 집중력 향상을 돕는 뽀모도로 기법과 에빙하우스 간격 복습 전략을 통해 단기간에 목표를 성취해 보세요."
    
    # meta 데이터 전송 시 언더스코어가 있는 버전과 없는 버전 둘 다 전송해서 필드가 확실히 채워지도록 함
    post_data = {
        "title": title,
        "content": content,
        "meta": {
            "_rank_math_focus_keyword": "시간관리 공부법",
            "rank_math_focus_keyword": "시간관리 공부법",
            ...
            "rank_math_permalink": "time-management-study-strategy"
        }
    }
    
    print(f"Updating WordPress post ID {post_id} with SEO corrections...")
    try:
        ...
        if response.status_code == 200:
            print("\n[SUCCESS] WordPress post ID 657 successfully updated!")
            print(f"Edit Link: {response.json().get('link')}")
        else:
            print(f"\n[FAILED] Update failed with status code: {response.status_code}")
            print(response.text)
    except Exception as e:
