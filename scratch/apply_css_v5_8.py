# -*- coding: utf-8 -*-
"""
모바일 상단 검색바의 바깥 배경색을 하단 내비게이션 바의 배경색(#fcfbfa)과 완벽하게 일치(Match)시키고, 
경계 테두리 역시 고급스럽게 다듬은 V5.8 CSS 갱신 스크립트
"""
import os
...
def update_css_v5_8():
    ...
    css_content = """/* ========================================================
   [V5.8 내추럴 웜 아이보리 화이트 테마 - 상단 검색바 컬러 하단탭과 매칭판] 
   ======================================================== */
...
@media (max-width: 768px) {
    ...
    /* 2. 상단 고정 검색바 높이(약 50px)만큼 본문 콘텐츠가 겹쳐서 가려지지 않도록 최상단 마진을 대폭 늘림 (중요) */
    #page {
        margin-top: 80px !important; /* 기존 55px에서 80px로 여백 확대하여 썸네일 가림 완벽 박멸 */
    }
...
/* ========================================================
   [모바일 상단 검색바 전용 스타일 정의 - 배경색을 하단 탭 바(#fcfbfa)와 동기화]
   ======================================================== */
.mobile-app-search-bar {
    ...
    background-color: #fcfbfa !important; /* 하단 5탭 바 배경색과 정확히 일치시킴 */
    padding: 10px 16px !important;
    border-bottom: 1px solid #e6e4e0 !important; /* 하단바 테두리와 일치 */
...
.mobile-app-search-bar .search-field {
    ...
    border: 1px solid #e6e4e0 !important; /* 바디 테두리톤에 맞춰 연하고 부드러운 선으로 교정 */
    background-color: #ffffff !important;
    color: #2b2927 !important;
...
"""
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(css_content)
    print("[SUCCESS] CSS compiled to V5.8. Synced search bar background color with bottom nav bar.")
