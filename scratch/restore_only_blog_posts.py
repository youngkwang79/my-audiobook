# -*- coding: utf-8 -*-
"""
소설 데이터는 100% 완전히 삭제(DELETE) 처리하여 배제하고,
오직 진짜 무림북.컴의 21대 도움되는 블로그 글들과 세무/금융 계산기 3종 (총 24개)만
works 테이블에 고화질 2:3 표지와 함께 무조건 복원(UPSERT)하는 오직 블로그 전용 스크립트.
"""
...
def run_restore_only_blog_posts():
    url = ""
    service_key = ""
    ...
    # 1. 기존 works 테이블에서 소설 및 찌꺼기 글 완전히 DELETE 날리기
    print("\n[STEP 1] Purging all novels and older mixed entries from works table...")
    ...
    print(f"\n--- [BLOG-ONLY RESTORATION COMPLETE] ---")
    print(f"Successfully restored all {restored_count} blog entries. Novels purged completely!")

if __name__ == "__main__":
    run_restore_only_blog_posts()
