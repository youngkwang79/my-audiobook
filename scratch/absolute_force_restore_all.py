# -*- coding: utf-8 -*-
"""
그 어떤 본문 텍스트 조건 검사(blockquote, 이모지, 작성자 체크 등)도 전면 배제하고,
무림북.컴 메인 사이트에 있어야 하는 진짜 오리지널 지식글/계산기 21개 품목 전체를 
 works 테이블에 고화질 2:3 R2 썸네일과 함께 무조건 100% 강제 복원(UPSERT)하는 스크립트.
(소설 연재작 4종은 철저히 보호)
"""
...
def run_absolute_force_restore():
    url = ""
    service_key = ""
    ...
    print("\n[STEP 1] Restoring all 21 original posts unconditionally...")
    ...
    for file_name in os.listdir(scratch_dir):
                ...
                if wid in blog_text_cover_map:
                    ...
                    try:
                        with urllib.request.urlopen(req_ups) as _:
                            print(f" ==> [FORCE RESTORED JS] ID: {wid} | Title: '{title}'")
                            restored_count += 1
        ...
        # PY 파일 복원
            ...
            if title_match and markdown_match and wid in blog_text_cover_map:
                ...
                try:
                    with urllib.request.urlopen(req_ups) as _:
                        print(f" ==> [FORCE RESTORED PY] ID: {wid} | Title: '{title}'")
                        restored_count += 1
                except Exception as e:
