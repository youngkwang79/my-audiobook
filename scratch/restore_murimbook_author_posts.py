# -*- coding: utf-8 -*-
"""
로컬 scratch/ 백업 스크립트들 중에서
본문 내부에 '작성자: murimbook' 또는 '작성자 : murimbook' 서명이 박혀있는 100% 진짜 무림북 원고들까지 엄선하여,
works 테이블에 고유 R2 2:3 세로형 표지를 씌워 최종 원상 복구(UPSERT)하는 스크립트.
"""
...
def run_murimbook_author_restoration():
    url = ""
    service_key = ""
    ...
    for file_name in os.listdir(scratch_dir):
        ...
        if file_name.startswith("register_") and file_name.endswith(".js"):
            ...
            if id_match and markdown_match and title_match:
                ...
                # '작성자 : murimbook' 또는 '작성자: murimbook' 검사
                has_sig = ("작성자: murimbook" in markdown) or ("작성자 : murimbook" in markdown) or ("작성자 : sun_writer" in markdown) or ("작성자: sun_writer" in markdown) or ("작성자 : sun-writer" in markdown) or ("작성자: sun-writer" in markdown)
                
                if wid in r2_exact_map and has_sig:
        ...
        # 2. PY 파일 복원
            ...
            if title_match and markdown_match and wid in r2_exact_map:
                ...
                # '작성자: murimbook' 또는 '작성자 : murimbook' 검사
                has_sig = ("작성자: murimbook" in markdown_cleaned) or ("작성자 : murimbook" in markdown_cleaned) or ("작성자 : sun_writer" in markdown_cleaned) or ("작성자: sun_writer" in markdown_cleaned) or ("작성자 : sun-writer" in markdown_cleaned) or ("작성자: sun-writer" in markdown_cleaned)
                ...
                if has_sig:
                    ...
                    try:
                        with urllib.request.urlopen(req_ups) as res_ups:
                            print(f" ==> [RESTORED PY] ID: {wid} | '{title}'")
                            restored_count += 1
                    except Exception as e:
                        print(f" ==> [ERROR] Failed to restore PY {wid}: {e}")

    print(f"\n--- [MURIMBOOK AUTHOR RESTORATION COMPLETE] ---")
    print(f"Successfully restored {restored_count} posts by murimbook / sun_writer.")

if __name__ == "__main__":
    run_murimbook_author_restoration()
