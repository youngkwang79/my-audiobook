# -*- coding: utf-8 -*-
"""
무림북.컴의 모든 오리지널 도움되는글 본문 최하단에
유저님이 소중하게 작성해 두셨던 꿀템 추천 영역 및 전용 8대 해시태그 블록을 
100% 한 줄의 누락도 없이 원본 백업에서 추출하여 works 테이블에 최종 복원(UPSERT)하는 스크립트.
"""
...
def run_full_content_tails_restoration():
    url = ""
    service_key = ""
    ...
    print("\n[STEP 1] Scanning scratch backups for full content with tails...")
    ...
    for file_name in os.listdir(scratch_dir):
                ...
                if wid in r2_exact_map:
                    ...
                    try:
                        with urllib.request.urlopen(req_ups) as _:
                            print(f" ==> [RESTORED JS WITH TAILS] ID: {wid} | '{title}'")
                            restored_count += 1
        ...
        # PY 파일 복원
            ...
            if title_match and markdown_match and wid in r2_exact_map:
                ...
                try:
                    with urllib.request.urlopen(req_ups) as _:
                        print(f" ==> [RESTORED PY WITH TAILS] ID: {wid} | '{title}'")
                        restored_count += 1
                except Exception as e:
                    print(f" ==> [ERROR] Failed to restore PY {wid}: {e}")

    print(f"\n--- [TAIL SECTIONS RESTORATION COMPLETE] ---")
    print(f"Successfully restored all {restored_count} target posts with their recommendations and hashtags.")

if __name__ == "__main__":
    run_full_content_tails_restoration()
