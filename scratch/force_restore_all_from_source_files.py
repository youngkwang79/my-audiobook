# -*- coding: utf-8 -*-
"""
로컬 scratch/ 폴더 내 모든 JS(*.js) 및 PY(*.py) 소스파일을 동적으로 열어
내부에 문자열로 기재되어 있던 모든 진짜 무림북.컴의 오리지널 칼럼 본문 전문을 자동 파싱 추출하고,
Supabase works 테이블에 고화질 2:3 R2 썸네일 표지와 함께 복원(UPSERT)하는 최후의 정화 마스터 스크립트.
(소설 및 워드프레스 수입 찌꺼기는 완전 배제)
"""
...
def run_all_sources_dynamic_restoration():
    url = ""
    service_key = ""
    ...
    print("\n[STEP 1] Scanning scratch/ folder for JS and PY script files dynamically...")

    for filename in os.listdir(scratch_dir):
        # 1. JS 파일 파싱 복원
        if filename.startswith("register_") and filename.endswith(".js"):
                ...
                if wid in blog_text_cover_map:
                    ...
                    try:
                        with urllib.request.urlopen(req_ups) as _:
                            print(f" ==> [DYNAMIC JS RESTORED] ID: {wid} | Title: '{title}'")
                            restored_count += 1
                    except Exception as e:
                        print(f" ==> [ERROR] JS Restore Fail: {e}")

        # 2. PY 파일 파싱 복원
        elif filename.startswith("upload_") and filename.endswith(".py"):
            file_path = os.path.join(scratch_dir, filename)
            wid = filename.replace("upload_", "").replace("_posts", "").replace("_wp", "").replace(".py", "")
            ...
            if title_match and markdown_match and wid in blog_text_cover_map:
                ...
                try:
                    with urllib.request.urlopen(req_ups) as _:
                        print(f" ==> [DYNAMIC PY RESTORED] ID: {wid} | Title: '{title}'")
                        restored_count += 1
                except Exception as e:
                    print(f" ==> [ERROR] PY Restore Fail: {e}")

    print(f"\n--- [ALL SOURCES RESTORATION COMPLETE] ---")
    print(f"Successfully restored all {restored_count} target blog entries from sources.")

if __name__ == "__main__":
    run_all_sources_dynamic_restoration()
