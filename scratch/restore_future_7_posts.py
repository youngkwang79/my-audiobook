# -*- coding: utf-8 -*-
"""
최신 7개 포스트를 다시 발행 전 상태인 예약 대기 상태(status='공개예정')로 돌리고,
현재 시각 기준 각각 1시간~7시간 뒤의 미래 시각으로 created_at 타임스탬프를 부여하여 
 works 테이블에 안전하게 복구 삽입(UPSERT)하는 스크립트.
(소설은 100% 완전 배제)
"""
...
def run_future_7_restoration():
    ...
    # 1. 워드프레스에서 글 70개 검색
    print("\n[STEP 1] Fetching WordPress posts pool for future 7...")
    wp_api_url = "https://blog.murimbook.com/wp-json/wp/v2/posts?status=any,trash,draft&per_page=70&_embed"
    ...
    # 2. 예약 복원 대상 및 정확한 물리 표지 2:3 이미지 맵 정의
    target_7_specs = [
        {
            "keyword": "근로장려금 자녀장려금",
            "hours_delay": 1,
    ...
    print("\n[STEP 2] Inserting target 7 posts into works table with status='공개예정' and future timeline...")
    base_time = datetime.datetime.now(datetime.timezone.utc)
    restored_count = 0

    for spec in target_7_specs:
        key = spec["keyword"]
        ...
        if matched_post:
            ...
            work_payload = {
                ...
                "last_pitch": None,
                "last_rate": None,
                "created_at": scheduled_iso # 미래 예약 타임스탬프 부여
            }
            ...
            try:
                with urllib.request.urlopen(req_ups) as res_ups:
                    ...
                    if res_payload:
                        print(f" ==> [FUTURE RESTORE SUCCESS] ID: {slug} | '{wp_title}'")
                        print(f"     -> Cover Path: {cover_url}")
                        print(f"     -> Scheduled Future Time: {scheduled_iso}")
                        restored_count += 1
            except Exception as e:
                print(f" ==> [ERROR] Failed to restore {wp_title}: {e}")

    print(f"\n--- [FUTURE 7 RESTORATION PROCESS COMPLETE] ---")
    print(f"Successfully restored {restored_count} posts back to future scheduling.")

if __name__ == "__main__":
    run_future_7_restoration()
