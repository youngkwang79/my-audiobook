...
# 📌 [수정완료]: 일반 명사(대한민국, 인터넷, 법률) 제거 및 도메인 핵심 전문 용어로만 구성하여 이탈률 최소화
WIKI_RESOURCES = [
    ...
    ("국민연금", "https://ko.wikipedia.org/wiki/%EA%B5%AD%EB%AF%BC%EC%95%A4%EA%B8%88"),
    ("실손보험", "https://ko.wikipedia.org/wiki/%EC%8B%A4%EC%86%90%EC%9D%98%EB%A3%8C%EB%B3%B4%ED%97%98"),
...
def self_critique_linter_validate(md_content, title, keyword):
    ...
    # 1. 소상공인 전기요금 자격 요건 1억 원 확대 팩트 검수 (제미나이 팩트체크 기반)
    if "전기요금" in title:
...
def make_html_template_clean(p, ch_key):
    ...
    # 📌 1, 2. 소상공인 전기요금 매출 요건 1억 원 상향 및 차감 방식 패치 (제미나이 피드백 반영)
    if "전기요금" in title:
        # 연 매출 6,000만 원 이하 -> 1억 원 이하
        ...
        facts[2] = facts[2].replace("6,000만 원 이하", "1억 원 이하").replace("6000만원 이하", "1억 원 이하")
        
        # 순차적 차감 -> 첫 고지서 일괄 차감/이월 소진으로 교체
...
{docs_list_html}  </ul>
    ...
    # 📌 [수정완료]: '공식 홈페이지 바로가기' + '바로가기' 중복 병합 방지 처리
    display_desc = desc
...
def main():
    print(f"=== [원고 라이터 v39] 소상공인 전기세 1억 원 한도 상향 및 팩트 교정 기동 ===")
...
"""
    ...
    if error_log:
        ...
        print(f"🎉 [대성공] 소상공인 매출 1억 요건과 일괄/이월 차감 방식 및 링크 정리까지 완벽 완료하여 최종 배포 성공!")
