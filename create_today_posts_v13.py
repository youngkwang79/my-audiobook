...
# 1. create_today_posts_v11에 등록된 풍부한 27개 포스트용 POSTS_CATALOG 데이터를 로드합니다.
# 이렇게 함으로써 유저가 어제 성공했던 실시간 황금 키워드 포스트 원천 텍스트가 단 1글자도 유실되지 않도록 철저히 수호합니다.
try:
...
# 2. 채널 카테고리별 특화 지식 서술 데이터베이스 (문맥 오염 방지용)
# 각 글의 카테고리에 딱 맞는 전문적인 서술문으로 5,000자 ~ 7,000자 대를 풍성하게 채웁니다.
KNOWLEDGE_BY_CATEGORY = {
    # 금융/재테크 채널 전용 (1, 2, 6, 7 채널)
    "FINANCE": [
...
# 3. HTML 치환기 (H2 ~ H6 완벽 지원 및 개별 인라인 CSS 폰트 스타일링 매핑)
def markdown_to_html(md_content):
    ...
    for line in lines:
        ...
        # H1 ~ H6 헤더 파서 보강 (H4 ~ H6 스타일 인라인 추가)
        ...
        if line_str.startswith('## '):
            h2 = line_str[3:].strip()
            clean_h2 = re.sub(r'^\d+\.\s+', '', h2) # 번호 제거
...
# 4. 개별 포스트용 5,000자 ~ 7,000자 조립 엔진 (H2 ~ H6 입체 가독성 구조 적용)
def make_html_template(p, ch_key):
    ...
    # 카테고리별 특화 벌크 텍스트 로드 (문맥 꼬임 완전 봉쇄)
  ...
  - {bullet3}<br>
    ...
    # 📌 5대 H2 본론 및 H3/H4/H5/H6 입체화 (가독성 분할 추가)
    body_sections = []
    wiki_links = [
    ...
    for i in range(5):
        ...
        # H3, H4, H5, H6를 동적으로 포함시켜 가독성 높인 입체 위계 구조로 본론 확장
        p1 = f"{core_fact}\n\n{base_texts[0]}"
  ...
  부정한 서식 기재나 자격 초과 이력이 관계 기관의 합동 전산 대조 결과에 따라 사후 적발될 시에는 이미 지급된 지원금 전액의 환수 명령과 함께 법률상 가산금 요율이 가중 소급 부과되므로, 반드시 팩트만을 성실하게 기재하셔야 합니다.
    ...
    # 📌 데이터 테이블 조립 (2행 3열 비교 테이블 구성)
    # 카테고리별 맞춤형 실질 정보 비교표 생성
    table_map = {
        "FINANCE": [
...
{docs_list_html}  </ul>
    ...
    # 📌 이미지 Alt 대체 텍스트 가이드 영역 (TOC 최상단 배치)
...
def main():
    print(f"=== [원고 라이터 v13-Hierarchy] 9대 채널 27개 고품격 입체 원고 컴파일 개시 ===")
    ...
    print(f"\n🎉 [대성공] H1 ~ H6 위계 분할이 보강된 27개 포스팅용 명품 HTML/MD 2차 복원 최종 완료!")
