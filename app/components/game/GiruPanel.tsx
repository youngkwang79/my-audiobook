...
export default function GiruPanel() {
  ...
  const favor =
    (game.npcFavors &&
      game.npcFavors[selectedNpc?.id || GIRU_NPCS[activeNpcIndex].id]) ||
    0;
  ...
  const npcStyles: Record<
    string,
    { scale: number; y: number; height: string; boxY: number }
  > = {
    yeonhwa: { scale: 0.7, y: -90, height: "100%", boxY: -240 },
    seolmae: { scale: 0.65, y: -120, height: "100%", boxY: -255 },
    chowoon: { scale: 0.7, y: -140, height: "100%", boxY: -270 },
    hongryeon: { scale: 0.8, y: -50, height: "100%", boxY: -105 },
    sohee: { scale: 0.8, y: -50, height: "100%", boxY: -110 },
    oldman: { scale: 0.8, y: -60, height: "100%", boxY: -120 },
  };

  const [dynamicStyles, setDynamicStyles] = useState<
    Record<string, { scale: number; y: number }>
  >({});
  ...
  const handleAction = async (npcId: string, actionId: string) => {
    ...
    if (actionId === "gift") {
      if (!hasAnyGift) {
        alert(
          "가지고 계신 선물이 없습니다. 기방에서 특별한 정보를 얻거나 퀘스트를 통해 선물을 획득해 보세요.",
        );
    ...
    if (actionId === "info" && npcId === "yeonhwa") {
      if (game.nightLimits?.infoTradeUsed) {
        alert("오늘은 이미 정보를 모두 거래했습니다. 내일 다시 시도해 주세요.");
        setIsProcessing(false);
        return;
      }
  ...
  const getFavorLabel = (npcId: string) => {
    ...
    if (!npc) return "알 수 없음";

    const threshold = [...npc.favorThresholds]
      .reverse()
      .find((t) => favor >= t.level);
    return threshold ? threshold.label : "호감 없음";
  };

  const handleGiveGift = (giftId: string) => {
    const res = interactGiru(
      selectedNpc?.id || GIRU_NPCS[activeNpcIndex].id,
      "gift",
      { giftId },
    );
    if (res.success) {
  ...
  const realmIndex = REALM_ORDER.indexOf(game.realm || "삼류");
  const isRealmLocked = false;
      ...
      >
        <AnimatePresence mode="wait">
          <motion.img
            ...
            src={
              selectedNpc
                ? npcBgs[selectedNpc.id] || "/images/giru_bg.png"
                : npcBgs[GIRU_NPCS[activeNpcIndex].id] || "/images/giru_bg.png"
            }
            alt="Giru Background"
            style={{
              width: "100%",
              height: "100%",
              objectFit: "cover",
              filter: selectedNpc ? "none" : "brightness(0.4) blur(1px)",
            }}
            onError={(e: any) => {
      ...
      >
        <h2
          style={{
            ...
            margin: "0",
            color: "#e0c3fc",
            textShadow: "0 0 15px rgba(224, 195, 252, 0.6)",
            fontWeight: 900,
          }}
        >
          🌙 월향루 (月香樓)
        ...
        {showPuzzle && (
          <GiruPuzzleGame
            ...
            onComplete={() => {
              ...
              setDialogue(
                "어머, 문제를 해결하셨군요! 당신의 눈썰미가 보통이 아니군요.",
              );
            }}
              ...
              <motion.div
                ...
                onDragEnd={(e, { offset }) => {
                  if (!isEditMode && Math.abs(offset.x) > 60) {
                    if (offset.x < -60 && activeNpcIndex < GIRU_NPCS.length - 1)
                      setActiveNpcIndex(activeNpcIndex + 1);
                    else if (offset.x > 60 && activeNpcIndex > 0)
                      setActiveNpcIndex(activeNpcIndex - 1);
                  }
                }}
                style={{
              ...
              >
                    ...
                    return (
                      <motion.img
                        ...
                        style={{
                          ...
                          objectFit: "contain",
                          filter:
                            "drop-shadow(0 0 20px rgba(224, 195, 252, 0.3))",
                        }}
                        initial={{ y: style.y, scale: style.scale }}
                        animate={{
                          y: isEditMode
                            ? style.y
                            : [style.y, style.y - 10, style.y],
                          scale: style.scale,
                        }}
                        transition={{
                          y: {
                ...
                >
                  ...
                  >
                    ...
                    >
                      {getFavorLabel(GIRU_NPCS[activeNpcIndex].id)} (
                      {(game.npcFavors &&
                        game.npcFavors[GIRU_NPCS[activeNpcIndex].id]) ||
                        0}
                      )
                    </div>
                ...
                {game.nightBuffs && game.nightBuffs.length > 0 && (
                  ...
                  >
                    적용 중인 효과 ({game.nightBuffs.length}){" "}
                    {showBuffs ? "닫기" : "보기"}
                  </button>
                )}
          ...
          >
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  {selectedNpc.name}와의 대화
                </div>
                <div
                  style={{ fontSize: "12px", color: "#e0c3fc", opacity: 0.8 }}
                >
            ...
            >
              ...
              {(() => {
                const npcQuests = GIRU_QUESTS.filter(
                  (q) => q.npcId === selectedNpc.id,
                );
                const activeQuestsList = (game.activeQuests || []).filter(
                  (q: any) =>
                    q.npcId === selectedNpc.id && q.status !== "rewarded",
                );
                const availableQuests = npcQuests.filter(
                  (nq) =>
                    !activeQuestsList.some(
                      (aq: any) => (aq.templateId || aq.id) === nq.id,
                    ),
                );
                ...
                const isAvailable = !activeQuestsList.some(
                  (aq: any) =>
                    (aq.templateId || aq.id) === (q.templateId || q.id),
                );
                ...
                return (
                  <div style={{ marginBottom: "15px", position: "relative" }}>
                    ...
                    >
                      ...
                      >
                        📜 {isAvailable ? "수락 가능한 의뢰" : "진행 중인 의뢰"}{" "}
                        ({currentIdx + 1}/{allDisplayQuests.length})
                      </div>
                    ...
                    >
                          ...
                          {isAvailable ? (
                            <button
                              onClick={() => {
                                ...
                                acceptQuest(q.id);
                                setDialogue(
                                  `[${q.title}] 의뢰를 수락하셨군요! 최선을 다해주시기 바랍니다.`,
                                );
                              }}
                              style={{
                                padding: "6px 12px",
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (tid === "q_yeonhwa_1") {
                                  const realmRewards =
                                    ROGUE_QUEST_REWARDS[game.realm] ||
                                    ROGUE_QUEST_REWARDS["삼류"];
                                ...
                                completeQuest(q.id);
                                setDialogue(
                                  `의뢰를 완수하셨군요! 정말 고생 많으셨습니다. 여기 약속한 보상입니다.`,
                                );
                              }}
                              style={{
                                padding: "6px 12px",
                      ...
                      >
                        💰 보상:{" "}
                        {(q.templateId || q.id) === "q_yeonhwa_1"
                          ? `${formatCompactNumber(ROGUE_QUEST_REWARDS[game.realm]?.gold || 100000)}원 / 도박권 ${ROGUE_QUEST_REWARDS[game.realm]?.token || 2}개`
                          : `${formatCompactNumber(q.reward?.gold || 0)}원 / 도박권 ${q.reward?.token || 0}개`}
                      </div>
                      ...
                      {q.status === "active" && (
                        <div style={{ marginTop: "10px" }}>
                          ...
                          >
                            <span>진행 중</span>
                            <span>
                              {q.currentCount || 0} / {q.targetCount || 0}
                            </span>
                          </div>
                          <div
                            style={{
          ...
          >
            ...
            >
              <motion.img
                ...
                src={`/images/giru/${selectedNpc.id}_secret_1.png`}
                style={{
                  maxWidth: "100%",
                  maxHeight: "100%",
                  objectFit: "contain",
            ...
            >
              ...
              >
                {GIRU_GIFT_ITEMS.map((gift) => {
                  ...
                  const isPreferred = (
                    selectedNpc || GIRU_NPCS[activeNpcIndex]
                  ).preferredGifts?.includes(gift.id);
                  return (
                    <motion.div
                      key={gift.id}
                      whileHover={{ scale: 1.02 }}
          ...
          >
            ...
            >
              ...
              <p style={{ fontSize: "12px", color: "#ccc" }}>
                은밀한 정보를 얻기 위해 월향루에 투자하세요. 투자 수준이
                높아질수록 더 많은 정보와 혜택을 얻을 수 있습니다.
              </p>
              ...
              >
                <div>현재 투자 레벨: {game.giruLevel || 1}</div>
                <div>
                  누적 투자 금액: {(game.giruInvestment || 0).toLocaleString()}{" "}
                  냥
                </div>
                {game.giruLevel < 10 ? (
                  ...
                  <div style={{ marginTop: "10px", color: "#4dff8a" }}>
                    최고 레벨 달성!
                  </div>
                )}
              </div>
              ...
              <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
                <button
                  onClick={() => {
                    ...
                    if (game.giruLevel >= 10) {
                      alert("이미 최고 레벨에 도달하셨습니다.");
                      return;
                    }
                    if (game.coins < nextCost) {
                      alert(
                        "투자금이 부족합니다. 조금 더 금화를 모아와 주세요.",
                      );
                      return;
                    }
                    useGameStore.setState((s: any) => ({
                      game: {
                        ...s.game,
                        coins: s.game.coins - nextCost,
                        giruLevel: (s.game.giruLevel || 1) + 1,
                        giruInvestment: (s.game.giruInvestment || 0) + nextCost,
                      },
                    }));
                    useGameStore.getState().triggerSave(true);
                    alert(
                      "투자가 완료되었습니다! 이제 월향루에서 더 많은 혜택을 누리실 수 있습니다.",
                    );
                  }}
                  style={{
                    flex: 1,
        ...
        {showSeolmaeBuffModal && (
          ...
          >
            ...
            >
              ...
              <p style={{ fontSize: "12px", color: "#ccc" }}>
                특별한 술자리를 통해 특별한 가호를 받을 수 있습니다. 하루에 단
                한 번만 가능합니다.
              </p>
              ...
              >
                {SEOLMAE_BUFFS.map((buff: any) => (
                  <button
                    key={buff.id}
                    onClick={() => {
                      const value = Math.floor(
                        Math.random() * (buff.max - buff.min) + buff.min,
                      );
                      useGameStore.setState((s: any) => ({
                        game: {
                          ...s.game,
                          nightBuffs: [
                            ...
                            {
                              id: buff.id,
                              name: `${buff.name} +${value}${buff.suffix}`,
                            },
                          ],
                        },
                      }));
                      alert(
                        `${buff.name} +${value}${buff.suffix} 가호를 받으셨습니다!`,
                      );
                  ...
                  >
                    {buff.name} ( {buff.min}
                    {buff.suffix} ~ {buff.max}
                    {buff.suffix} )
                  </button>
                ))}
              </div>
              <button
                onClick={() => setShowSeolmaeBuffModal(false)}
                style={{
                  width: "100%",
                  padding: "10px",
                  background: "#555",
      ...
      <AnimatePresence>
        {showInfoTradeModal && (
          ...
          >
            ...
            >
              <h3 style={{ margin: "0 0 10px 0", color: "#e0c3fc" }}>
                비밀 정보 거래
              </h3>
              <p style={{ fontSize: "12px", color: "#ccc" }}>
                어떤 정보가 궁금하신가요? 가격에 따라 정보의 가치가 달라집니다.
              </p>
                ...
                {["low", "mid", "high", "special"].map((tier) => {
                  ...
                  return (
                    <button
                      key={tier}
                      onClick={() => {
                        const res = interactGiru("yeonhwa", "info", {
                          infoTier: tier,
                        });
                        if (res.success) {
                          setDialogue(res.message);
              ...
              >
                {rewardData.gold > 0 && (
                  ...
                  >
                    ...
                    >
                      +{formatCompactNumber(rewardData.gold)}
                    </span>
                  </div>
                )}
                {rewardData.token > 0 && (
                ...
                {rewardData.exp > 0 && (
                  ...
                  >
                    ...
                    >
                      +{formatCompactNumber(rewardData.exp)}
                    </span>
                  </div>
                )}
              </div>
        ...
        >
          <motion.div
            ...
            style={{
              ...
              border: `1px solid ${isRealmLocked ? "#ffd700" : "#e0c3fc"}`,
              borderRadius: "20px",
              padding: "16px 20px",
          ...
          >
            ...
            <h3
              style={{
                fontSize: "20px",
                color: isRealmLocked ? "#ffd700" : "#e0c3fc",
                fontWeight: 900,
                marginBottom: "10px",
              }}
            >
              {isRealmLocked ? "출입 제한" : "밤에만 운영"}
            </h3>
            <p
              style={{
                fontSize: "14px",
            ...
            >
              {isRealmLocked ? (
                <>
                  "월향루는 수준 높은 손님만을 모시는 곳입니다. <br />
                  최소한{" "}
                  <span style={{ color: "#ffd700", fontWeight: 900 }}>
                    삼류(三流) 이상
                  </span>
                  은 되셔야 발걸음을 하실 수 있습니다. <br />
                  더욱 정진하여 자격을 증명하시길 바랍니다."
                </>
              ) : (
                <>
                  "월향루의 기녀들은 낮에는 휴식을 취하고 있습니다. <br />
                  달이 뜨고 밤이 찾아와야 화려한 무대가 시작됩니다. <br />
                  <span style={{ color: "#e0c3fc", fontWeight: 900 }}>밤</span>
                  이 되면 다시 찾아주세요."
                </>
              )}
            </p>
            <div style={{ marginTop: "20px", fontSize: "11px", color: "#888" }}>
              {isRealmLocked ? (
                <>
                  현재: <span style={{ color: "#fff" }}>{game.realm}</span> /
                  요구: 일류 이상
                </>
              ) : (
                <>
                  현재는 <span style={{ color: "#ffd700" }}>낮</span>{" "}
                  시간입니다.
                </>
              )}
            </div>
