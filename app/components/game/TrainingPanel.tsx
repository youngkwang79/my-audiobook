...
export default function TrainingPanel() {
  ...
  const showInnVictoryEffect = useGameStore(
    (s: any) => s.game.showInnVictoryEffect,
  );
  const currentMissionTitle = useGameStore(
    (s: any) => s.game.currentMissionTitle,
  );
  ...
  useEffect(() => {
    if (
      ...
      tutorialProgress?.currentStepId === "start_training"
    ) {
    ...
    return () => {
      if (showInnVictoryEffect) {
        useGameStore.setState((s: any) => ({
          game: { ...s.game, showInnVictoryEffect: false },
        }));
      }
    };
  }, []);
  ...
  const [showBreakthroughSuccessEffect, setShowBreakthroughSuccessEffect] =
    useState(false);
  const [breakthroughSuccessRealm, setBreakthroughSuccessRealm] = useState<
    string | null
  >(null);
  ...
  const [hitEffects, setHitEffects] = useState<
    {
      ...
      x: number;
      y: number;
      type: "slash" | "blue-slash" | "flash";
    }[]
  >([]);
  const [textStrikes, setTextStrikes] = useState<
    { id: any; char: string; x: number; y: number; groupId: number }[]
  >([]);
  const [activeOilText, setActiveOilText] = useState<string | null>(null);
  const [rewardFloats, setRewardFloats] = useState<
    { id: number; gold: number; rep: number; x: number; y: number }[]
  >([]);
  ...
  const currentRealmName =
    currentRealmIndex >= 0 ? realmKeys[currentRealmIndex] : "필부";
  const nextRealmName =
  ...
  const upgradeTargetLabel = isRealmBreakthrough
    ? `${nextRealmName} 경지`
    : `${star + 1}성`;
  ...
  const getPopupContent = () => {
    ...
    if (
      msg.includes("수련 성과") ||
      (msg.includes("HP +") && msg.includes("MP +"))
    ) {
      return {
        reward: msg.replace("💪 ", ""),
      };
  ...
  const lastTrainingReward = useGameStore((s: any) => s.game.lastTrainingReward);
  useEffect(() => {
    if (lastTrainingReward) {
      const newFloat = {
        id: Date.now() + Math.random(),
        gold: lastTrainingReward.gold,
        rep: lastTrainingReward.rep,
        x: 12 + (Math.random() * 6 - 3), // 주인공 머리 위 위치 (x: ~12%)
        y: 55 + (Math.random() * 4 - 2), // 주인공 머리 위 위치 (y: ~55%)
      };
      setRewardFloats((prev) => [...prev, newFloat].slice(-5));
      setTimeout(() => {
        setRewardFloats((prev) => prev.filter((f) => f.id !== newFloat.id));
      }, 1500);
    }
  }, [lastTrainingReward]);
  ...
  const performHit = () => {
    ...
    setShowPrompt(false);
    const { pendingInnEntry, timingMission, tutorialProgress } =
      useGameStore.getState().game;

    // 튜토리얼 중 특정 조작이 필요한 단계(대장간 등)에서는 허수아비 타격 방지
    const isRestrictedTutorialStep =
      tutorialProgress?.isActive &&
      ...
      ![
        ...
        "master_unlock",
        "inn_event",
      ].includes(tutorialProgress.currentStepId);

    if (
      showBreakthroughPopup ||
      pendingInnEntry ||
      timingMission?.available ||
      isRestrictedTutorialStep
    )
      return;

    const totalCritRate = useGameStore.getState().getTotalCritRate
      ? useGameStore.getState().getTotalCritRate()
      : 5;
    ...
    const equippedWeapons = ownedWeapons.filter((w: any) =>
      equippedIds.includes(w.id),
    );
    ...
    if (triggerSkill) {
      if (skillWeapons.length > 0) {
        ...
        const bestEqSkill = skillWeapons.sort(
          (a: any, b: any) =>
            b.equipmentSkill!.multiplier - a.equipmentSkill!.multiplier,
        )[0].equipmentSkill!;
        eqSkillName = bestEqSkill.name;
        eqSkillDamage = totalAtk * bestEqSkill.multiplier;
      } else if (gameState.learnedSkills.length > 0) {
        const bestSkill = [...gameState.learnedSkills].sort(
          (a: any, b: any) =>
            ((b as any).multiplier || 0) - ((a as any).multiplier || 0),
        )[0];
        // [수정] 무공 발동 시 실제 내공 소모 로직 추가
        const skData =
          MARTIAL_COMPENDIUM.find(
            (m: any) => m.name === bestSkill.name && m.factionName === faction,
          ) || bestSkill;
        ...
        if (gameState.mp >= mpCost) {
          ...
          const learned = martialArtsSkills.find(
            (ms: any) =>
              ms.skillId === (skData as any).id ||
              ms.skillId === (skData as any).skillId,
          );
          ...
          martialSkillDamage =
            totalAtk * baseMultiplier * refineMult * damageMultiplier;
    ...
    const critDmgMult = useGameStore.getState().getTotalCritDmg
      ? useGameStore.getState().getTotalCritDmg() / 100
      : 1.5;
    const baseFinalDmg = isCritical
      ? Math.floor(totalAtk * critDmgMult)
      : totalAtk;
    ...
    if (oilRes.buffsTriggered.length > 0) {
      ...
      if (triggeredName) {
        if (oilEffectTimeoutRef.current)
          clearTimeout(oilEffectTimeoutRef.current);
        setActiveOilText(triggeredName + "!");
        oilEffectTimeoutRef.current = setTimeout(
          () => setActiveOilText(null),
          4000,
        );
      }
    }
  ...
  const handleConfirmBreakthrough = () => {
    const targetRealm = isRealmBreakthrough
      ? nextRealmName
      : `${realm} ${star + 1}성`;
  ...
  return (
    ...
    >
        ...
        >
          <div
            style={{
              ...
              filter: "blur(22px)",
              animation:
                "breakthroughAuraSpin 7s linear infinite, breakthroughAuraPulse 2.4s ease-in-out infinite",
              pointerEvents: "none",
            }}
          />
          ...
          <div
            style={{
              ...
              filter: "blur(42px)",
              animation:
                "breakthroughAuraSpinReverse 11s linear infinite, breakthroughAuraWave 3.8s ease-in-out infinite",
              pointerEvents: "none",
            }}
          />
          ...
          <div
            style={{
              ...
              borderRadius: "9999px",
              background:
                "radial-gradient(circle, rgba(255,255,255,0.34) 0%, rgba(255,255,255,0.10) 35%, rgba(255,255,255,0) 72%)",
              filter: "blur(10px)",
          ...
          >
            ...
            >
              <img
                src={
                  FACTIONS.find((f: any) => f.name === faction)?.characterImages
                    ?.ready || "/warrior.png"
                }
                alt="Character"
                style={{
                  ...
                  objectFit: "contain",
                  filter:
                    "brightness(2) contrast(1.5) drop-shadow(0 0 20px #ffd700)",
                }}
              />
            </div>
            ...
            >
              ...
              {"\n"}
              {upgradeTargetLabel}(으)로 {isRealmBreakthrough ? "돌파" : "승급"}
              하시겠습니까?
            </div>
            ...
            >
              <button
                ...
                style={{
                  ...
                  animation:
                    "breakthroughConfirmPulse 1.3s ease-in-out infinite",
                }}
              >
                다음으로
              </button>
      ...
      {(pendingInnEntry || timingMission?.available) &&
        !useGameStore.getState().game.tutorialProgress.isActive && (
          <div
          ...
          >
            ...
            >
              ...
              >
                <div
                  style={{
                    height: "100%",
                    background:
                      "linear-gradient(90deg, #ff0000, #ff6666, #ff0000)",
                    borderRadius: "5px",
                    width: "100%",
              ...
              <button
                onClick={() =>
                  useGameStore.setState((s: any) => ({
                    game: { ...s.game, activeTab: "inn" },
                  }))
                }
                style={{
                  background:
                    "linear-gradient(135deg, #ff3333 0%, #990000 100%)",
                  color: "white",
                  border: "none",
                  padding: "12px 30px",
                  borderRadius: "30px",
          ...
          >
            <div
              style={{
                ...
                fontWeight: 1000,
                background:
                  "linear-gradient(180deg, #FFFFFF 0%, #FFEAA7 30%, #FF9900 60%, #D32F2F 100%)",
      ...
      >
          ...
          >
            <div
              style={{
                ...
                fontStyle: "italic",
                textShadow:
                  "0 0 10px #ff4500, 0 0 20px #ff4500, 0 0 40px #ff0000",
                letterSpacing: "-1px",
        ...
        {movementBuff && (
          ...
          >
            <div
              style={{
                background:
                  "linear-gradient(90deg, transparent, rgba(0,242,255,0.7), transparent)",
                padding: "4px 0",
          ...
          >
            <div
              style={{
                ...
                lineHeight: "1.25",
                textShadow:
                  "0 0 6px rgba(0,255,120,0.85), 0 0 12px rgba(0,0,0,0.7)",
                whiteSpace: "nowrap",
              }}
            >
            ...
            <div
              style={{
                ...
                lineHeight: "1.25",
                textShadow:
                  "0 0 6px rgba(80,180,255,0.85), 0 0 12px rgba(0,0,0,0.7)",
                whiteSpace: "nowrap",
                paddingLeft: "8px",
              }}
            >
          ...
          {rewardFloats.map((rf) => (
            <div
              ...
              style={{
                ...
                gap: "2px",
                zIndex: 200,
                animation: "floatUpWide 1.5s ease-out forwards",
              }}
            >
              <div
                style={{
                  ...
                  fontWeight: 900,
                  fontSize: "14px",
                  textShadow: "1px 1px 2px #000, 0 0 8px rgba(255,215,0,0.5)",
                }}
              >
                금화 +{formatCompactNumber(rf.gold)}
              </div>
              <div
                style={{
                  color: "#ffcc00",
                  fontWeight: 800,
                  fontSize: "11px",
                  textShadow: "1px 1px 2px #000, 0 0 6px rgba(255,204,0,0.4)",
                }}
              >
                명성 +{formatCompactNumber(rf.rep)}
              </div>
            </div>
          ))}
        </div>
          ...
          {textStrikes.map((st: any) => (
            <div
              ...
              style={{
                ...
                color: "#fff",
                textShadow:
                  "0 0 10px #fff, 0 0 20px #ff4500, 0 0 40px #ff0000, 0 0 60px #ffd700",
                animation:
                  "textStrikeAnim 1.8s cubic-bezier(0.18, 0.89, 0.32, 1.28) forwards",
                pointerEvents: "none",
                zIndex: 170,
                WebkitTextStroke: "1px #ffd700",
              }}
        ...
        >
          {showPrompt && !pendingInnEntry && !timingMission.available && (
            <div
              style={{
                ...
                textAlign: "center",
                textShadow:
                  "2px 2px 0px #000, 0 0 20px rgba(255,215,0,0.8), 0 0 40px rgba(255,69,0,0.6)",
                pointerEvents: "none",
          ...
          <div
            style={{
              ...
              transform: isShaking
                ? "rotate(10deg) scale(0.95) translateX(8px)"
                : "rotate(0deg)",
            ...
            style={{
              ...
              animation:
                characterAction === "attack"
                  ? "warriorStrike 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275)"
                  : "none",
            }}
          >
            <img
              src={(() => {
                ...
                if (characterAction === "attack") {
                  return (
                    fInfo?.characterImages?.attack || "/warrior_attack.png"
                  );
                }
              ...
              style={{
                ...
                filter: movementBuff
                  ...
                  : attackMultiplier > 1
                    ? "drop-shadow(0 0 15px #ff4500) brightness(1.2)"
                    : "drop-shadow(10px 15px 12px rgba(0,0,0,0.9))",
              }}
            />
          </div>
        </div>
        ...
        >
          {missionSlide === 0
            ? `임무: ${dummyKills}/${questTarget}`
            : `무뢰배: ${totalDummyKills < 300 ? totalDummyKills : dummyKills}/${totalDummyKills < 300 ? 300 : questTarget}`}{" "}
          | 총합: {totalDummyKills}
        </div>
      </div>
      ...
      >
        ...
        >
          {(() => {
            const activeQuests =
              useGameStore.getState().game.activeQuests || [];
            const ongoingQuests = activeQuests.filter(
            ...
            if (currentSlideIndex === 0) {
              ...
              return (
                ...
                >
                  객잔 무뢰배 추격 (현재:{" "}
                  {totalDummyKills < 300 ? totalDummyKills : dummyKills} /{" "}
                  {totalDummyKills < 300 ? 300 : questTarget})
                </span>
              );
            } else {
          ...
          <div
            style={{
              width: `${Math.min(
                ...
                (() => {
                  ...
                  if (currentSlideIndex === 1)
                    return totalDummyKills < 300
                      ? totalDummyKills / 300
                      : dummyKills / questTarget;
              ...
              background:
                missionSlide %
                  (2 +
                    (useGameStore.getState().game.activeQuests || []).filter(
                      (q: any) => q.status === "active",
                    ).length) >=
                2
          ...
          {[...Array(12)].map((_: any, i: number) => (
            <div
              ...
              style={{
                ...
                height: "40px",
                background:
                  "radial-gradient(circle, #fff 0%, rgba(255,50,50,0.8) 50%, transparent 100%)",
                borderRadius: "50%",
      ...
      <style jsx global>{`
        ...
        @keyframes character3DPanDarkMild {
          0% {
            transform: perspective(1000px) rotateY(-3deg) translateX(-53%)
              scale(1);
            filter: brightness(0.9);
          }
          50% {
            transform: perspective(1000px) rotateY(3deg) translateX(-47%)
              scale(1.05);
        ...
        >
          ...
          >
            ...
            <span style={{ color: "#fff" }}>
              {combatAnalysis.log
                .reduce((s: number, e: any) => s + e.damage, 0)
                .toLocaleString()}
            </span>
          </div>
          <div
          ...
          >
            ...
            <span style={{ color: "#fbbf24" }}>
              {(
                combatAnalysis.log.reduce(
                  (s: number, e: any) => s + e.damage,
                  0,
                ) / Math.max(1, 10 - combatAnalysis.timeLeft)
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  {combatAnalysis.results.totalDamage.toLocaleString()}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                >
                  {Math.floor(combatAnalysis.results.dps).toLocaleString()}
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                <tbody>
                  {Object.entries(combatAnalysis.results.breakdown).map(
            ...
            {Object.keys(combatAnalysis.results.skillDetails).length > 0 && (
              <div style={{ marginBottom: "20px" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                  {Object.entries(combatAnalysis.results.skillDetails).map(
                    ([name, dmg]: [string, any]) => (
                      <div
                        key={name}
                        style={{
            ...
            <button
              onClick={() =>
                useGameStore.setState({
                  combatAnalysis: { ...combatAnalysis, results: null },
                })
              }
              style={{
                width: "100%",
