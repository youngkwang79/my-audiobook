...
export default function InventoryPanel(props: Props) {
  ...
  const [selectedMedicineId, setSelectedMedicineId] =
    useState<ConsumableId | null>(null);
  const [lastTapTime, setLastTapTime] = useState(0);
  const [lastTapId, setLastTapId] = useState<string | null>(null);

  const [draggedMedicineId, setDraggedMedicineId] =
    useState<ConsumableId | null>(null);
  ...
  const equippedCount = Object.values(equippedGear ?? {}).filter(
    Boolean,
  ).length;

  const selectedItems = ownedWeapons.filter(
    (item: any) => item.slot === selectedSlot,
  );
  ...
  const handleItemTap = (item: OwnedWeapon) => {
    ...
    if (now - lastTapTime < 600 && lastTapId === item.id) {
      ...
      tapTimeoutRef.current = setTimeout(() => {
        ...
        if (game.tutorialProgress?.currentStepId === "select_item_inventory") {
          setTutorialStep("click_equip_button");
        } else if (
          game.tutorialProgress?.currentStepId === "select_infused_item"
        ) {
          setTutorialStep("check_final_infused_options");
        }
        tapTimeoutRef.current = null;
      }, 150);
    }
  ...
  const onGearTouchEnd = (e: React.TouchEvent, item: OwnedWeapon) => {
    ...
    if (swipeOffset > 100) {
      const price = item.name.includes("[패왕]")
        ? 40000000
        : Math.floor(item.price * 0.25);
      if (
        confirm(
    ...
    if (slotIdxAttr !== null && slotIdxAttr !== undefined) {
      ...
      const dist = Math.sqrt(
        Math.pow(dragPos.x - dragOrigin.x, 2) +
          Math.pow(dragPos.y - dragOrigin.y, 2),
      );
      if (dist < 10) {
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <div
                    style={{
                      fontSize: 16,
                      filter: active ? "drop-shadow(0 0 3px #ffd778)" : "none",
                    }}
                  >
                    {meta.icon}
                  </div>
          ...
          {isMaterialsSelected ? (
            ...
            >
              {(Object.keys(consumables) as ConsumableId[])
                .filter((id: any) => consumables[id] > 0)
            ...
            >
              {selectedItems.map((item: any, idx: number) => {
                ...
                return (
                  <button
                    key={item.id}
                    id={idx === 0 ? "inv-item-list-first" : undefined}
                    onClick={() => {
                      handleItemTap(item);
                    }}
                    style={{
              ...
              {selectedItems.length === 0 && (
                ...
                >
                  ...
                  보유 중인 {slotLabel(selectedSlot)}이(가) 없습니다.
                </div>
              )}
            </div>
          )}
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: consumableId
                    ? "1.5px solid #ffd700"
                    : "1.5px dashed rgba(255,215,0,0.3)",
                  background: "rgba(255,255,255,0.03)",
                  display: "grid",
                  placeItems: "center",
                  position: "relative",
              ...
              >
                {consumableId ? (
                  ...
                  <span style={{ fontSize: 16, color: "rgba(255,215,0,0.4)" }}>
                    +
                  </span>
                )}
              </button>
          ...
          >
            ...
            >
              {quickSlots[selectingSlot] && (
                <button
                  onClick={() => {
                    (useGameStore.getState() as any).useConsumable(
                      quickSlots[selectingSlot],
                    );
                  ...
                  style={{
                    padding: 10,
                    borderRadius: 8,
                    background: "linear-gradient(135deg, #ff8c8c, #ff4b4b)",
                    color: "#fff",
                    border: "none",
                    cursor: "pointer",
                    fontSize: 12,
              ...
              <button
                onClick={() => {
                  setQuickSlot(selectingSlot, null);
                  setSelectingSlot(null);
                }}
                style={{
                  padding: 10,
              ...
              {(Object.keys(consumables) as ConsumableId[])
                .filter(
                  (id: any) => consumables[id] > 0 && !id.startsWith("oil_"),
                )
                .map((id: any) => (
                  <button
                    key={id}
                    onClick={() => {
                      setQuickSlot(selectingSlot, id);
                      setSelectingSlot(null);
                    }}
                    style={{
                      padding: "8px 12px",
              ...
              {(Object.keys(consumables) as ConsumableId[]).every(
                (id: any) => consumables[id] <= 0,
              ) && (
                <div
                  style={{
                    textAlign: "center",
          ...
          >
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              <div style={{ fontSize: 40 }}>
                {getPotionIcon(selectedMedicineId)}
              </div>
              <div>
                <div
                  style={{ fontSize: 18, fontWeight: 900, color: "#ffd700" }}
                >
                  {getPotionName(selectedMedicineId)}
                </div>
                <div style={{ fontSize: 12, color: "#aaa" }}>
                  보유 수량: {consumables[selectedMedicineId]}개
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <button
                onClick={() => {
                  (useGameStore.getState() as any).sellConsumable(
                    selectedMedicineId,
                  );
                  if (consumables[selectedMedicineId] <= 1)
                    setSelectedMedicineId(null);
                }}
                style={{
                  padding: "10px",
      ...
      {popupItem && (
        ...
        >
          ...
          >
            ...
            >
              {Number(popupItem.attackBonus) > 0 && (
                <div style={{ color: "#ff4d4d", fontWeight: "bold" }}>
                  기본 공격력 +{popupItem.attackBonus}
                </div>
              )}
              {Number(popupItem.hpBonus) > 0 && (
                <div style={{ color: "#a8ff7e", fontWeight: "bold" }}>
                  기본 생명력 +{popupItem.hpBonus}
                </div>
              )}
              {Number(popupItem.mpBonus) > 0 && (
                <div style={{ color: "#4d94ff", fontWeight: "bold" }}>
                  기본 내공 +{popupItem.mpBonus}
                </div>
              )}

              {/* 무작위 옵션 표시 */}
              {popupItem.randomOptions &&
                popupItem.randomOptions.length > 0 && (
                  <div
                    style={{
                      marginTop: 4,
                      paddingTop: 4,
              ...
              {popupItem.setName && (
                ...
                >
                  ...
                  >
                    🧩 세트:{" "}
                    {(SYNERGY_CONFIG as any)[popupItem.setName]?.label ||
                      popupItem.setName}
                  </div>
                  {(() => {
                    const counts = (
                    ...
                    const setDesc =
                      (SYNERGY_CONFIG as any)[popupItem.setName!]
                        ?.description || "";
              ...
              {popupItem.touchMultiplier && (
                <div style={{ color: "#7ee7ff", fontWeight: "bold" }}>
                  수련 효율 +
                  {((popupItem.touchMultiplier - 1) * 100).toFixed(0)}%
                </div>
              )}
              {popupItem.expMultiplier && (
                <div style={{ color: "#a8ff7e", fontWeight: "bold" }}>
                  수련 경험치 +
                  {((popupItem.expMultiplier - 1) * 100).toFixed(0)}%
                </div>
              )}
              {popupItem.paralyzeChance && (
                <div style={{ color: "#e07eff", fontWeight: "bold" }}>
                  마비 확률 {popupItem.paralyzeChance}% (
                  {popupItem.paralyzeDuration! / 1000}초)
                </div>
              )}
    ...
    return "즉시 발동";
            ...
            >
              <button
                id="inv-equip-btn"
                onClick={() => {
                  handleEquipFromPopup();
                  const { game, setTutorialStep } =
                    useGameStore.getState() as any;
                  if (
                    game.tutorialProgress.currentStepId === "click_equip_button"
                  ) {
                    setTutorialStep("goto_forge_refine");
                  }
                }}
                style={{
                  flex: 1,
                  ...
                  fontWeight: "bold",
                  border:
                    popupItem.id === selectedEquippedId
                  ...
                  color:
                    popupItem.id === selectedEquippedId ? "#fff" : "#2b1d00",
                  cursor: "pointer",
                }}
              >
              ...
              <button
                onClick={() => {
                  if (
                    confirm(
                      `${popupItem.name}을(를) 판매하시겠습니까? (판매가: ${formatCompactNumber(Math.floor(popupItem.price * 0.25))} 냥)`,
                    )
                  ) {
              ...
              <button
                onClick={() => {
                  ...
                  if (
                    confirm(
                      `${popupItem.name}을(를) 분해하시겠습니까?\n획득 재료: 상급 재료 ${materials}개`,
                    )
                  ) {
                    (useGameStore.getState() as any).dismantleItem(
                      popupItem.id,
                    );
                    setPopupItem(null);
                  }
...
function getPotionDesc(id: ConsumableId) {
  ...
  if (id === "oil_atk_3")
    return "2% 확률로 공격력이 3배로 증가합니다. (지속 5초)";
  if (id === "oil_crit_3")
    return "2% 확률로 치명타 피해가 3배로 증가합니다. (지속 5초)";
  if (id === "oil_thunder")
    return "5% 확률로 500%의 강력한 뇌전 피해를 입히고 기절시킵니다.";
  if (id === "oil_poison")
    return "5% 확률로 적의 방어력을 50% 약화시킵니다. (지속 10초)";
  if (id === "oil_bleed")
    return "5% 확률로 매초 최대 체력의 10%만큼 지속 피해를 입힙니다.";
  if (id === "oil_eva_3")
    return "5% 확률로 회피율이 3배로 증가합니다. (지속 10초)";
  if (id === "oil_def_3")
    return "7% 확률로 받는 모든 피해가 50% 감소합니다. (지속 10초)";
  if (id === "oil_reflect")
    return "7% 확률로 받은 피해의 200%를 반사합니다. (지속 10초)";
  if (id === "oil_vajra")
    return "5% 확률로 모든 공격을 무시하는 금강불괴 상태가 됩니다. (지속 5초)";
  if (id === "oil_vampire") return "5% 확률로 입힌 대미지의 50%를 흡혈합니다.";
  if (id === "oil_speed_3")
    return "5% 확률로 공격 속도가 2배로 증가합니다. (지속 10초)";
  if (id === "oil_luck_3")
    return "5% 확률로 전리품의 등급이 상승할 확률이 크게 증가합니다. (지속 10초)";
  if (id === "oil_clarity") return "8% 확률로 모든 상태이상을 즉시 해제합니다.";
  if (id === "oil_eye") return "15% 확률로 적의 공격을 반드시 회피합니다.";
  if (id === "oil_demon")
    return "2% 확률로 적을 즉시 일격필살(1000% 대미지)합니다.";
  if (id === "oil_triple_hit") return "5% 확률로 3배의 연타 공격을 발동합니다.";
  if (id === "oil_formless")
    return "3% 확률로 적 현재 체력의 10%를 즉시 삭감합니다.";
  if (id === "oil_blessed")
    return "모든 능력치가 소폭 상승하는 축복을 받습니다.";
  if (id === "paewang_box")
    return "패왕의 권능이 서린 보물입니다. 최고의 무구를 획득할 수 있습니다.";
  if (id === "stone_box_tujeon")
    return "장비 강화에 필요한 현철 강화석 30개를 획득합니다.";
  if (id === "rare_box_tujeon")
    return "무작위 부위의 희귀 등급 장비를 획득합니다.";
  if (id === "night_gear_box")
    return "현재 경지에 맞는 무작위 영웅 등급 야행 장비를 획득합니다.";
  if (id === "gear_piece_bundle")
    return "장비 제작에 필요한 야행 장비 조각 5개를 획득합니다.";
  if (id === "manual_fragment_bundle")
    return "무공 비급 제작에 사용되는 비급 조각들을 무작위로 획득합니다.";
