      ...
      >
        ...
        >
          <div>
            ...
            >
              <span>{userFaction.name === "무당" ? "☯️" : "⚔️"}</span>
              장경각 (藏經閣)
            </h2>
            <p style={{ margin: "4px 0 0 0", fontSize: "12px", color: "#888" }}>
              {userFaction.name === "무당"
                ? "무당파의 심오한 무공 비급이 보관된 곳입니다."
                : "강호의 수많은 비급과 무공이 기록되어 있습니다."}
            </p>
          </div>
          <div style={{ textAlign: "right" }}>
            ...
            >
              {getMaterialCount("무학 심득").toLocaleString()} pt
            </div>
          </div>
        </div>

        <nav style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
          {(["compendium", "refinement", "synthesis", "equip"] as const).map(
            (tab) => (
              <button
                key={tab}
        ...
        >
          ...
          <AnimatePresence mode="wait">
            {activeTab === "compendium" && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                style={{ display: "grid", gap: "10px", minHeight: "100.1%" }}
              >
            ...
            >
              ...
              >
                ...
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  ...
                  {compendiumView.skills.filter((s) => s.unlocked).length ===
                    0 && (
                    <p style={{ fontSize: "13px", color: "#666" }}>
                      먼저 무공을 습득해야 연마가 가능합니다.
                    </p>
                  )}
                </div>
              </div>
              ...
              {selectedSkill && (
                ...
                >
                  <div style={{ marginBottom: "20px" }}>
                    <h2
                      style={{ fontSize: "24px", color: "#fff", margin: "0" }}
                    >
                      {selectedSkill.name}
                    </h2>
                    <div
                    ...
                    >
                      {Array.from({ length: 10 }).map((_, i) => (
                        <span
                          key={i}
                          style={{
                            opacity: i < selectedSkill.stars ? 1 : 0.2,
                            marginRight: "2px",
                          }}
                        >
                          ★
                  ...
                  >
                    <div>
                      ...
                      >
                        x{selectedSkill.multiplier} →{" "}
                        <span style={{ fontWeight: "bold" }}>
                          x
                          {(
                            selectedSkill.multiplier *
                            getRefineBonusMultiplier(selectedSkill.stars + 1)
                          ).toFixed(2)}
                        </span>
                      </div>
                    </div>
                    <div>
                      ...
                      <div style={{ fontSize: "18px", color: "#88ff88" }}>
                        {getRefineBonusText(selectedSkill.stars)}
                      </div>
                    </div>
                  </div>
                  ...
                  {selectedSkill.category === "movement" &&
                    (() => {
                      const currentBuff = getMovementBuff(
                        game.faction,
                        selectedSkill.stars,
                      );
                      const nextBuff = getMovementBuff(
                        game.faction,
                        Math.min(10, selectedSkill.stars + 1),
                      );
                      ...
                      const getStatLabel = (key: string) => {
                        const labels: Record<string, string> = {
                          ...
                          freeze: "빙결",
                          weakness: "약점 타격",
                          enemyAspdReduction: "적 공속 감소",
                          reflect: "피해 반사",
                          def: "방어력",
                          manaShield: "내공 보호막",
                          manaEfficiency: "내공 효율",
                          healPerTouch: "타격 시 흡수",
                          dmgReduction: "피해 감소",
                          invincible: "무적 성질",
                          instantHeal: "즉시 회복",
                          critDmgMult: "치명 배율",
                      ...
                      const formatVal = (key: string, val: number) => {
                        ...
                        if (
                          ["invincible", "freeze", "manaShield"].includes(key)
                        )
                          return "발동";
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            {currentBuff &&
                              Object.entries(currentBuff.multipliers).map(
                                ([key, val]) => (
                                  <div
                                    key={key}
                                    style={{
                        ...
                        >
                          <span
                            style={{
                              color:
                                game.coins >=
                                getRefineGoldCost(selectedSkill.stars)
                                  ? "#ffd700"
                                  : "#ff4444",
                            }}
                          >
                          ...
                          <span
                            style={{
                              color:
                                game.wisdom >=
                                getRefineWisdomCost(selectedSkill.stars)
                                  ? "#66ccff"
                                  : "#ff4444",
                            }}
                          >
          ...
          {activeTab === "synthesis" && (
            <motion.div
              ...
              style={{ display: "grid", gap: "15px", minHeight: "100%" }}
            >
              <div
                style={{
                  padding: "10px",
              ...
              >
                ...
                <p style={{ fontSize: "12px", color: "#888" }}>
                  두 가지 무공의 극한(7성 이상)에 도달하면 새로운 비기를 창조할
                  수 있습니다.
                </p>
              </div>

              {recipes.map((recipe) => {
                const isLearned = game.learnedSkills.some(
                  (s: any) => s.name === recipe.resultName,
                );
                // 재료 체크
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        )[recipe.resultGrade] || recipe.resultGrade}
                      </div>
                    </div>
                    ...
                    >
                      {ingredients.map((ing, i) => (
                        ...
                        >
                          <div style={{ color: ing.ok ? "#8f8" : "#f88" }}>
                            {ing.name}
                          </div>
                          <div style={{ fontSize: "9px" }}>
                            {recipe.requiredStars}성 필요
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    >
                      <div style={{ fontSize: "12px", color: "#888" }}>
                        ...
                        <span style={{ color: "#ffd700" }}>
                          {recipe.goldCost.toLocaleString()}냥
                        </span>{" "}
                        /{" "}
                        <span style={{ color: "#66ccff" }}>
                          {recipe.wisdomCost}심득
                        </span>
                      </div>
                      <button
                        ...
                        style={{
                          ...
                          fontWeight: "bold",
                          cursor:
                            isLearned || !canCraft ? "default" : "pointer",
                          transition: "all 0.2s",
                        }}
                      >
                        {isLearned
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: "12px", color: "#888" }}>
                    장착 슬롯:{" "}
                    <span style={{ color: userFaction.theme.accent }}>
                      {(game.masterDuel.equippedSkillIds || []).length} / 4
                    </span>
                  </div>
                </div>
                ...
                >
                  전투에서 사용할 무공을 최대 4개까지 선택할 수 있습니다.
                  <br />
                ...
                <div style={{ display: "grid", gap: "10px" }}>
                  {compendiumView.skills
                    ...
                    .map((skill) => {
                      const isEquipped = (
                        game.masterDuel.equippedSkillIds || []
                      ).includes(skill.name);
                      return (
                        <div
                          key={skill.id}
...
function CompendiumCard({
  ...
  if (!isCompleted && !isLocked) {
    ...
    currentRes = {
      fragments: getMaterialCount(reqs.fragmentId),
      materials: getMaterialCount(reqs.materialId || "일반 재료"),
      bonds: getMaterialCount(`${reqs.bondId} 인연`),
      gearFrags: getMaterialCount("장비 조각"),
      divineShards: getMaterialCount("신기 파편"),
      insights: getMaterialCount("무학 심득"),
      coins: game.coins,
    };

    canCraft =
      currentRes.fragments >= reqs.requiredFragments &&
    ...
    >
        ...
        {!isLocked ? (
          <div
            style={{
              ...
              background: "rgba(0,0,0,0.2)",
              borderRadius: "8px",
              borderLeft: `3px solid ${isCompleted ? accent : "#444"}`,
            }}
          >
            {skill.description}
          </div>
          ...
          >
            <span>🔒</span>
            <span>
              단서 부족: {skill.realm} 경지에 도달하고 기루에서 정보를
              수집하십시오.
            </span>
          </div>
        )}
        ...
        {!isLocked && (
          ...
          >
            <span>
              ⚡ 소모 내공:{" "}
              <span style={{ color: "#66ccff" }}>
                {skill.mpCost?.toLocaleString() || "0"}
              </span>
            </span>
            <span>
              ⚔️ 기본 위력:{" "}
              <span style={{ color: "#ff8888" }}>{skill.multiplier}배</span>
            </span>
          </div>
        )}
      </div>
      ...
      <div style={{ marginLeft: "24px", minWidth: "180px" }}>
        {isLocked ? (
          ...
          >
            <div
              style={{ fontSize: "11px", color: "#444", fontWeight: "bold" }}
            >
              획득 불가
            </div>
            <button
              onClick={() =>
                alert(
                  "기루(妓樓)에서 해당 문파의 비급 정보를 구매하여 단서를 확보하십시오.",
                )
              }
              style={{
                padding: "8px 16px",
          ...
          >
            ...
            {reqs && (
              <div
                id="library-materials-container"
                style={{
                  display: "grid",
              ...
              >
                <ResourceRow
                  icon="📜"
                  label={getManualFragmentDisplayName(
                    skill.factionName,
                    skill.name,
                  )}
                  current={currentRes.fragments}
                  required={reqs.requiredFragments}
                />
                <ResourceRow
                  icon="💎"
                  ...
                  current={currentRes.materials}
                  required={reqs.requiredMaterials}
                />
                <ResourceRow
                  icon="⚙️"
                  label="장비 조각"
                  current={currentRes.gearFrags}
                  required={reqs.requiredGearFragments}
                />
                <ResourceRow
                  icon="✨"
                  label="신기 파편"
                  current={currentRes.divineShards}
                  required={reqs.requiredDivineWeaponShards}
                />
                <ResourceRow
                  icon="🤝"
                  label={getFactionBondDisplayName(skill.factionName)}
                  current={currentRes.bonds}
                  required={reqs.requiredBonds}
                />
                <ResourceRow
                  icon="💡"
                  label="무학 심득"
                  current={currentRes.insights}
                  required={reqs.requiredInsights}
                />
                <ResourceRow
                  icon="💰"
                  label="제작 비용"
                  current={currentRes.coins}
                  required={reqs.goldCost}
                  isCost
                />
              </div>
            )}
    ...
    <div
      style={{
        ...
        background: isOk
          ...
          : "rgba(255, 82, 82, 0.05)",
        borderRadius: "6px",
        border: `1px solid ${isOk ? "#4caf5044" : "#ff525222"}`,
      }}
    >
      <span
        style={{
      ...
      >
        {isCost
          ? formatCompactNumber(required)
          : `${formatCompactNumber(current)}/${formatCompactNumber(required)}`}
      </span>
    </div>
  );
}
