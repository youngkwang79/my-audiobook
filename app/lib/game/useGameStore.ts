...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  getUpgradeCost: (key: keyof GameSaveData["statUpgrades"]) => {
    ...
    // 비용 증가폭 4% (1.04)
    return Math.floor(500 * Math.pow(1.04, count));
  },
  getReputationCost: (key: keyof GameSaveData["statUpgrades"]) => {
    ...
    // 비용 증가폭 3% (1.03)
    return Math.floor(100 * Math.pow(1.03, count));
  },
  ...
  getMultiUpgradeCost: (key, count, mode) => {
    ...
    for (let i = 0; i < count; i++) {
       ...
       if (mode === 'gold') {
         // 비용 증가폭 4% (1.04)
         totalCost += Math.floor(500 * Math.pow(1.04, upgradeCount));
       } else {
         // 비용 증가폭 3% (1.03)
         totalCost += Math.floor(100 * Math.pow(1.03, upgradeCount));
       }
    }
    return totalCost;
  },
