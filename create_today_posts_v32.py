...
def make_html_template_clean(p, ch_key):
  ...
  - {bullet3}<br>
    ...
    for i in range(5):
        ...
        # 📌 [대수술]: 분량 늘리기용 '유령 문장'인 ext_p를 본문 전체에서 싹 다 제거!
        # H2 아래에는 오직 진짜 사실 관계를 다룬 100% 진짜 사실 문장(fact_spaced)만 남깁니다.
        p1 = fact_spaced
...
def main():
    print(f"=== [원고 라이터 v32-CleanUryungPost] 9대 채널 27개 글 전체 유령 문장 싹 다 삭제 및 교정 개시 ===")
    ...
    for ch_key, folder_prefix in PREFIX_MAP.items():
        ...
        # 1. 3개 포스트 글 본문만 유령 문장 제거하여 덮어쓰기 (thumbnail_prompts.html 보전)
...
"""
            ...
            print(f"  ✅ Re-written (100% Fact Only): post{no:02d}.md ({char_count} chars) & post{no:02d}.html")
            
        print(f"  💡 Preserved: thumbnail_prompts.html")
            
    print(f"\n🎉 [대성공] 9대 채널 전체 27개 글에서 중복/유령 문장을 100% 걷어내고 순수 팩트 원고로 덮어쓰기 완료!")
