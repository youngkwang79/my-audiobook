...
def main():
    print(f"=== [원고 라이터 v15-Convenient] 9대 채널 27개 고품격 입체 원고 컴파일 개시 ===")
    ...
    for ch_key, folder_prefix in PREFIX_MAP.items():
        ...
        for p in posts_data_list:
            ...
            # 2. 초편리 원클릭 복사 스크립트가 내장된 html 본문 파일 컴파일
...
"""
            ...
            print(f"  ✅ Compiled H1~H6 + Convenient Script: post{no:02d}.md ({char_count} chars) & post{no:02d}.html")
            
    print(f"\n🎉 [대성공] 원클릭 복사 스크립트가 탑재된 27개 HTML/MD 최종 컴파일 배포 완료!")
