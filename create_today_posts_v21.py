...
def build_thumbnail_prompts_html(posts, ch_key):
    ...
    for p in posts:
        ...
        if ch_key == "6_murimbook":
            ...
            image_text_guide = f"✍️ [인쇄 합성 텍스트]: \"{title}\" (배경 중앙부에 굵은 명조체로 직접 인쇄 합성)"
        else:
            final_prompt = base_prompt
            image_text_guide = f"✍️ [인쇄 합성 텍스트]: \"{keyword}\" (배경 중앙 또는 하단에 깔끔한 서체로 인쇄 합성)"
...
<body style="margin: 0; padding: 30px; background-color: #f8fafc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
  ...
  <div style="max-width: 800px; margin: 0 auto;">
    ...
    <!-- 상단 헤더 타이틀 -->
    <div style="background: linear-gradient(135deg, #1e3a8a, #3b82f6); color: white; padding: 30px; border-radius: 12px; margin-bottom: 30px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1);">
      <h1 style="margin: 0 0 10px 0; font-size: 24px; font-weight: 800;">🎨 오늘의 블로그 대표 썸네일 생성기</h1>
      <p style="margin: 0; font-size: 14px; opacity: 0.9;">채널: {ch_name_korean} | 날짜: {DATE_STR}</p>
    </div>

    <!-- 안내 메시지 박스 -->
    <div style="background: #eff6ff; border-left: 4px solid #3b82f6; padding: 15px; border-radius: 4px; margin-bottom: 25px; font-size: 14px; color: #1e3a8a; line-height: 1.6;">
      <strong>💡 사용 요령:</strong> 아래 각 카드에서 [프롬프트 복사] 버튼을 누른 뒤 Midjourney 또는 DALL-E에 입력하여 썸네일을 생성하세요. <br>
...
def main():
    print(f"=== [원고 라이터 v21-IndependentThumb] 9대 채널 27개 독립 썸네일 HTML 컴파일 개시 ===")
...
"""
    ...
    print(f"\n🎉 [대성공] 각 날짜 폴더마다 독립형 'thumbnail_prompts.html' 파일 생성이 완벽 완료되었습니다!")
