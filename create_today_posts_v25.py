...
KNOWLEDGE_BY_CATEGORY = {
    ...
    "ENTERTAINMENT": [
        ...
        "아울러 비대면 OTT 어플리케이션을 통한 간편 스트리밍 전송 방식을 이용할 경우, 오프라인 극장 지사를 직접 내방하는 번거로움 없이도 본인 스마트폰 인증만 거치면 단 3분 만에 다시보기 처리가 완결되어 매우 편리합니다. 다만 해외 우회 접속이나 불법 유출본을 다운로드할 경우에는 저작권법 침해 대조 조항에 따라 service 이용 정지 처리가 내려짐을 유의하셔야 합니다."
    ]
}
...
def split_text_by_sentence(txt):
    """
    긴 텍스트 덩어리를 온점(.) 단위로 파싱하여, 각 문장마다 개행(\n\n)을 주입해
    가독성이 뛰어난 짧은 단락 형태로 분리해 줍니다.
    """
...
def make_html_template_clean(p, ch_key):
    ...
    # 📌 팩트 체크 및 도입부 등 긴 문맥 전체 가독성 향상 분할 처리
    ...
    for txt in base_texts:
        ...
        # 📌 덩어리 지식 텍스트를 문장 단위로 쪼개기
        spaced_txt = split_text_by_sentence(t)
        processed_base_texts.append(spaced_txt)
...
def build_thumbnail_prompts_html(posts, ch_key):
    ...
    for p in posts:
        ...
        if ch_key == "6_murimbook":
            ...
            image_text_guide = f"🎯 [썸네일 인쇄 후킹 문구]: <span style='color: #d97706; font-weight: bold;'>\"{hooking_copy}\"</span>"
        else:
            ...
            image_text_guide = f"🎯 [썸네일 인쇄 후킹 문구]: <span style='color: #2563eb; font-weight: bold;'>\"{hooking_copy}\"</span>"
...
def main():
    print(f"=== [원고 라이터 v25-SplitReadability] 9대 채널 27개 가독성 극대화 줄바꿈 컴파일 개시 ===")
...
"""
    ...
    print(f"\n🎉 [대성공] 각 날짜 폴더마다 문장별 강제 줄바꿈 및 여백 처리가 완벽히 적용되었습니다!")
