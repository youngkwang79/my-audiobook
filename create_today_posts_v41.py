...
def get_dynamic_docs_and_table(title, keyword):
    if "출산휴가" in title:
        ...
        table = [
            ...
            ["차감 적용 방식", "전기요금 청구서에서 20만 원 일괄/이월 차감", "대표자 신청 계좌로 20만 원 직접 현금 환급"]
        ]
    elif "국민연금" in title:
        # 📌 3. 국민연금 가입이력 요약원 서류 전산 조회 생략 안내 패치
        docs = [
            "조기노령연금 지급 신청서 (국민연금공단 제출용)",
            "신청인 신분증 사본 및 지급 수령 계좌 통장 사본",
            "국민연금 가입이력 요약원 (공단 전산망 자동 조회로 제출 생략 가능)"
        ]
        table = [
...
def get_category_key(ch_key, title):
    # 📌 1. 국민연금(PENSION) 카테고리 추가
    if "국민연금" in title or "노령연금" in title:
        return "PENSION"
...
def self_critique_linter_validate(md_content, title, keyword):
    ...
    # 📌 국민연금 예산소진 및 부정수급 환수 오인 팩트 린터 검수
    if "국민연금" in title:
        if "예산 조기 소진" in md_content or "예산 소진" in md_content:
            errors.append("[국민연금 오류] 국민연금은 법정 연금 제도이므로 '예산 조기 소진' 템플릿이 검출되면 기각됩니다.")
        if "지급된 지원금 전액의 환수 명령" in md_content or "가산금 요율이 가중 소급" in md_content:
            errors.append("[국민연금 오류] 국민연금 조기수령에 보조금 부정수급 성격의 위압적 가산금 문구가 검출되었습니다.")
            
    # 소상공인 전기요금 자격 요건 1억 원 확대 팩트 검수
    if "전기요금" in title:
        if "6,000만 원 이하" in md_content or "6000만원 이하" in md_content:
            errors.append("[전기요금 팩트오류] 최신 기준 소상공인 전기요금 특별지원 대상 연 매출액 요건은 1억 원 이하입니다.")
        if "매달 순차적" in md_content:
            errors.append("[전기요금 팩트오류] 20만 원 지원 방식은 일괄 차감 및 다음 달 이월 형태입니다.")
    ...
    # 실손보험 관련 자가 모순 체크
    if "실손" in title or "보험" in title:
        if "지원금" in md_content or "정부 지원" in md_content or "환수 명령" in md_content or "gasan" in md_content or "가산금" in md_content:
...
def make_html_template_clean(p, ch_key):
    ...
    # 📌 소상공인 전기요금 매출 요건 1억 원 상향 및 일괄 차감 패치
    if "전기요금" in title:
        intro = intro.replace("6,000만 원 이하", "1억 원 이하").replace("6000만원 이하", "1억 원 이하")
        closing = closing.replace("6,000만 원 이하", "1억 원 이하").replace("6000만원 이하", "1억 원 이하")
        
        facts[0] = facts[0].replace("6,000만 원 이하", "1억 원 이하").replace("6000만원 이하", "1억 원 이하")
        facts[1] = facts[1].replace("6,000만 원 이하", "1억 원 이하").replace("6000만원 이하", "1억 원 이하")
        
        facts[2] = facts[2].replace("매달 순차적으로 차감 적용되며", "지원 결정 후 첫 고지서부터 총 20만 원이 모두 소진될 때까지 일괄 차감 및 잔액 이월 방식으로 제공되며")
        facts[2] = facts[2].replace("매달 순차적으로 차감 적용", "첫 고지서 일괄 차감 및 잔액 이월 적용")
        facts[2] = facts[2].replace("매달 순차적 차감 적용 방식으로 제공됩니다", "지원 결정 후 첫 고지서부터 총 20만 원이 모두 소진될 때까지 일괄 차감 및 잔액 이월 방식으로 제공됩니다")
        facts[2] = facts[2].replace("매달 순차적", "첫 고지서 일괄 차감")
        
    # 📌 배우자 출산휴가 최신 법정 팩트 교정 패치
    if "출산휴가" in title:
        facts[1] = facts[1].replace("최초 5일 분", "10일 분 전체").replace("5일 분의", "10일 분 전체의")
        facts[2] = facts[2].replace("최대 1년간 매칭 지원합니다", "휴가를 사용한 기간(최대 10일)에 비례하여 일할 계산(월 20만 원 기준) 지급합니다")
        facts[2] = facts[2].replace("최대 1년간 매칭 지원", "휴가 기간(최대 10일)에 비례한 지급")
  ...
  - {bullet3}<br>
    ...
    for i in range(5):
        ...
        # 📌 1, 2. 국민연금(PENSION) 카테고리 전용 정밀 템플릿 장착 (제미나이 팩트체크 기반)
        if cat_key == "PENSION":
            if i == 0:
  ...
  본 제도는 개개인의 재정 상황에 맞춰 수령 시기를 선택할 수 있도록 설계되었습니다. 연금 개시 전 공단 홈페이지나 내 곁에 국민연금 앱에서 본인의 예상 감액률과 수령액을 미리 조회해 보시는 것을 권장합니다.
</div>"""
            elif i == 3:
  ...
  조기노령연금 수급 중 '소득이 있는 업무(기준 소득금액 초과)'에 종사하게 되면, 조기연금 지급이 즉시 정지되고 일반 노령연금 개시 연령이 될 때까지 연금을 받으실 수 없습니다. 따라서 신청 전 본인의 향후 소득 계획을 반드시 점검하셔야 합니다.
</div>"""
        elif cat_key == "MATERNITY":
            if i == 0:
                sec_text += f"""\n\n<div style="background-color: #e3f2fd; border-left: 5px solid #1e88e5; padding: 15px; margin: 20px 0; border-radius: 4px; line-height: 1.8;">
  <strong style="color: #1565c0; display: block; font-size: 15px; margin-bottom: 5px;">💡 전문 지식 Tip</strong>
  ...
  부정한 서식 기재나 자격 초과 이력이 관계 기관의 합동 전산 대조 결과에 따라 사후 적발될 시에는 이미 지급된 지원금 전액의 환수 명령과 함께 법률상 가산금 요율이 가중 소급 부과되므로, 반드시 팩트만을 성실하게 기재하셔야 합니다.
    ...
    # 📌 4. 위키 링크 전면 제거 (제미나이 팩트체크 기반 이탈률 최소화 조치 실행)
    # WIKI_RESOURCES 매칭 루프 제거
...
{docs_list_html}  </ul>
    ...
    display_desc = desc
    if not display_desc.endswith("바로가기"):
        display_desc = f"{display_desc} 바로가기"
  ...
  - <a href="{url}" target="_blank" rel="noopener noreferrer" style="color: #1565c0; text-decoration: underline; font-weight: bold;">{display_desc}</a>
</div>"""
...
def main():
    print(f"=== [원고 라이터 v41] 국민연금 수급정지 조건 및 서류생략 팩트 반영 ===")
...
"""
    ...
    if error_log:
        ...
        print(f"🎉 [대성공] 국민연금 조기수령 조건, 지급정지 조건 및 위키링크 클리닝까지 완벽히 클리어하여 배포 성공!")
