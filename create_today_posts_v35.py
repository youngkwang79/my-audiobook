...
def make_html_template_clean(p, ch_key):
    ...
    # 📌 3. 실손보험 주관 기관 및 약국 영수증 패치 (제미나이 팩트체크 기반 완벽 교정)
    if "실손" in title or "보험" in title:
  ...
  - {bullet3}<br>
    ...
    for i in range(5):
        ...
        # 📌 [대보안]: 민간 도메인(HEALTH, TECH, ENTERTAINMENT)은 정부 지원금/예산소진 템플릿 제거하고 맞춤형 템플릿 주입!
...
def main():
    print(f"=== [원고 라이터 v35] 실손보험 등 민간 도메인 팩트 무결성 패치 개시 ===")
...
"""
    ...
    if error_log:
        ...
        print(f"🎉 [대성공] 9대 채널 전체 27개 원고가 Critique Linter 실손보험 조건부 팩트체크를 통과하고 최종 배포되었습니다!")
