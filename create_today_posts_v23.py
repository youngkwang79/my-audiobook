...
KNOWLEDGE_BY_CATEGORY = {
    ...
    "TECH": [
        "본 소프트웨어 및 OS 최적화의 구체적 실무 진행 단계를 들여다보면, 사용 환경에 따른 호환 하드웨어 대조 검증과 백그라운드 리소스 수급 이력에 대한 런타임 조회 등 다각적인 시스템 조율 과정이 포함되어 있음을 알 수 있습니다. 무엇보다 시스템의 동작 건전성을 선제적으로 수호하기 위해 가용한 메모리 버팔 할당 한도를 매칭 설계해 둔 엔진 조항이므로, 부적격 설정 옵션이나 호환 탈락 사유를 꼼꼼하게 사전 점검하지 않으면 최종 패치 적용 과정에서 부결되어 성능 저하의 손해를 입게 됩니다. 설정을 진행하려는 대상자는 관련 지침 매뉴얼을 면밀히 탐독해 두는 것이 실익 면에서 극도로 유리합니다.",
    ...
    "ENTERTAINMENT": [
        ...
        "아울러 비대면 OTT 어플리케이션을 통한 간편 스트리밍 전송 방식을 이용할 경우, 오프라인 극장 지사를 직접 내방하는 번거로움 없이도 본인 스마트폰 인증만 거치면 단 3분 만에 다시보기 처리가 완결되어 매우 편리합니다. 다만 해외 우회 접속이나 불법 유출본을 다운로드할 경우에는 저작권법 침해 대조 조항에 따라 서비스 이용 정지 처리가 내려짐을 유의하셔야 합니다."
    ]
}

# 📌 예쁜 일러스트 그림체 썸네일 베이스 프롬프트
...
# 📌 보조 함수들 배치 우선 정의 (NameError 완벽 회피)
...
def build_thumbnail_prompts_html(posts, ch_key):
    ...
    분야별 썸네일 예쁜 그림체 프롬프트와 후킹 텍스트가 조화를 이루며, 
    후킹 텍스트 지시어(Korean text)가 영어 프롬프트 내부에 명시적으로 직접 포함되어 인쇄되도록 엮습니다.
    """
    cat_key = get_category_key(ch_key)
    ...
    for p in posts:
        ...
        # 📌 썸네일 프롬프트 본문 내에 후킹 문구 지시어 직접 동적 삽입!
        if ch_key == "6_murimbook":
            # 2:3 비율 무림북 스타일 + 후킹 문구 영어 지시어 주입
            ...
            image_text_guide = f"🎯 [썸네일 인쇄 후킹 문구]: <span style='color: #d97706; font-weight: bold;'>\"{hooking_copy}\"</span> (프롬프트 내에 한글 텍스트 그리기가 자동으로 내장되었습니다)"
        else:
            # 16:9 비율 일반 채널 + 후킹 문구 영어 지시어 주입
            ...
            image_text_guide = f"🎯 [썸네일 인쇄 후킹 문구]: <span style='color: #2563eb; font-weight: bold;'>\"{hooking_copy}\"</span> (프롬프트 내에 한글 텍스트 그리기가 자동으로 내장되었습니다)"
...
def main():
    print(f"=== [원고 라이터 v23-EmbeddedHookingPrompt] 9대 채널 27개 썸네일 내장형 후킹 컴파일 개시 ===")
...
"""
    ...
    print(f"\n🎉 [대성공] 각 날짜 폴더마다 후킹 프롬프트가 내장된 'thumbnail_prompts.html' 파일 생성이 완벽 완료되었습니다!")
