...
def markdown_to_html(md_content):
    ...
    for line in lines:
        ...
        parsed_line = re.sub(r'\*\frac{(.*?)}{(.*?)}\*\*|<b>(.*?)</b>|<strong>(.*?)</strong>', r'<strong>\1\2\3\4</strong>', parsed_line)
...
def make_html_template(p, ch_key):
  ...
  - {bullet3}<br>
    ...
    for i in range(5):
        ...
        # 📌 기계적 숫자 번호("1.1", "1.1.1") 대신 시맨틱 문자 A, B, C, D와 이모티콘 접두어 매치 수호
        sub_h3 = f"### 🔹 A. {keyword} - {clean_h} 세부 실무 및 자격 쟁점 분석"
...
def main():
    print(f"=== [원고 라이터 v17-SemanticSymbol] 9대 채널 27개 소제목 기호화 컴파일 개시 ===")
...
"""
            ...
            print(f"  ✅ Compiled H1~H6 + Semantics: post{no:02d}.md ({char_count} chars) & post{no:02d}.html")
            
    print(f"\n🎉 [대성공] 세부소제목 기호화 및 원클릭 복사 스크립트 탑재형 27개 HTML/MD 최종 컴파일 배포 완료!")
