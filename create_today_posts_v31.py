...
# 📌 3차 타깃 채널 3개 (7_wordpress, 8_blogger, 9_naver_연예 = 총 9개 글)
TARGET_CHANNELS = ["7_wordpress", "8_blogger", "9_naver_연예"]
...
def get_dynamic_docs_and_table(title, keyword):
    if "크롬" in title:
        ...
        docs = [
            ...
            "해당 근로자 임금대장 및 근로계약서 사본",
            "동료 대체 업무 분담 지정서 및 수당 지급 증빙 자료"
        ]
        table = [
...
def main():
    print(f"=== [원고 라이터 v31-Phase3] 3차 타깃 3개 채널 총 9개 원고 콤팩트 재생성 개시 ===")
...
"""
    ...
    print(f"\n🎉 [대성공] 3차 타깃 3개 채널(총 9개 원고)의 본문 재생성이 중복과 팩트 오류를 모두 해결하여 완결되었습니다!")
